...
import org.junit.*; // import annotations
 ...
 */
@Ignore
public class DBMSMetadataTest extends TestCase {
  ...
  public static Test suite() {
    TestSuite suite = new TestSuite(DBMSMetadataTest.class);
    return suite;
  }
  ...
  public void testGetSourceCode() throws Exception {
    ...
    char[] charArray = new char[1024];
    int readChars = 0; 
    while(( readChars = result.read(charArray)) > 0 )
    {
      System.out.println("Reader.read(CharArray)=="+readChars);
      stringBuilder.append(charArray, 0, readChars);
    }
  ...
  public void testGetSchemas() throws Exception {
    ...
    // Not until Java6 ResultSet matchedSchemas = result.getMetaData().getSchemas(catalog, schemasPattern) ;
    ...
    System.out.format("testURI=%s,\ngetSchemasList()=%s\n,getSourceCodeTypesList()=%s\n,getSourceCodeNmesList()=%s\n",testURI
                      , testURI.getSchemasList()
                      , testURI.getSourceCodeTypesList()
                      , testURI.getSourceCodeNamesList()
                     );
