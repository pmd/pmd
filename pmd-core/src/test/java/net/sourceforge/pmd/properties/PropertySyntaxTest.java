...
class PropertySyntaxTest extends RulesetFactoryTestBase {
    ...
    private PropertyDescriptor<?> defineProperty(String propDef) {
        ...
        descriptors.removeAll(
                setOf(Rule.VIOLATION_SUPPRESS_REGEX_DESCRIPTOR, Rule.VIOLATION_SUPPRESS_XPATH_DESCRIPTOR));
        return descriptors.get(0);
    }

    private static String contextForPropertyDef(String propDef) {
        return rulesetXml(dummyRule(properties(propDef)));
    }

    @Test
    void testPropDefXml() {
        PropertyDescriptor<?> prop = defineProperty(propertyDefWithValueAttr("pname", "pdesc", "String", "strvalue"));
    ...
    void testNumericPropDefWithoutBounds() {
        ...
        PropertyDescriptor<?> prop =
                defineProperty("<property name='pname' description='d' type='Integer' value='4' />");
    ...
    void testNumericPropDefWithMinBound() {
        // https://github.com/pmd/pmd/issues/1204
        PropertyDescriptor<?> prop =
                defineProperty("<property name='pname' description='d' type='Integer' value='4' min='1' />");
        ...
        assertEquals(
                ...
                prop.serializer().getConstraints().get(0).getConstraintDescription());
    }
    ...
    void testNumericPropDefWithMaxBound() {
        // https://github.com/pmd/pmd/issues/1204
        PropertyDescriptor<?> prop =
                defineProperty("<property name='pname' description='d' type='Integer' value='4' max='6' />");

        assertEquals(
                "Should be smaller or equal to 6",
                prop.serializer().getConstraints().get(0).getConstraintDescription());
    }
    ...
    void testNumericPropDefWithMaxAndMin() {
        // https://github.com/pmd/pmd/issues/1204
        PropertyDescriptor<?> prop =
                defineProperty("<property name='pname' description='d' type='Integer' value='4' max='6' min='2' />");
    ...
    void testNumericPropConstraintViolated() {
        // https://github.com/pmd/pmd/issues/1204
        assertCannotParse(
                contextForPropertyDef("<property name='pname' description='d' type='Integer' value='4' max='1' />"));
