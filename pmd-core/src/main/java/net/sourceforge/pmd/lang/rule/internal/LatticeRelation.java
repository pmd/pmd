...
class LatticeRelation<K, @NonNull V, C> {
     ...
     *              there is a cycle
     */
    private void addSucc(@NonNull Set<LNode> preds, final K k, final @Nullable V val, Set<K> seen) {
        if (seen.contains(k)) {
        ...
        LNode n = nodes.get(k);
        if (n != null) {
            // the leaf and its successors already exist
            link(preds, n, val);
            return;
        }
    ...
    private void link(@NonNull Set<LNode> preds, LNode n, @Nullable V val) {
    ...
    private void putDontCheckParams(@NonNull K key, @Nullable V value) {
        addSucc(new LinkedHashSet<>(), key, value, new LinkedHashSet<>());
    }
    ...
    private abstract class LNode { // "Lattice Node"
        ...
        abstract void addAsSuccessorTo(Set<LNode> preds);
    ...
    private final class QueryNode<A> extends LNode {
        ...
        @Override
        void addAsSuccessorTo(Set<LNode> preds) {
            preds.forEach(n -> {
                n.transitiveSuccs.add(this);
