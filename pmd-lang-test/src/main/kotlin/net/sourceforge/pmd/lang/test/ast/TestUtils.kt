...
private fun <N, V> assertWrapper(callable: KCallable<N>, expected: V, asserter: (N, V) -> Unit) {
    ...
    fun formatName() =
        "::" +
            callable.name.removePrefix("get").replaceFirstChar { it.lowercase(Locale.getDefault()) }
    ...
    try {
        ...
        if (e.message?.contains("expected:") == true) {
            ...
            throw AssertionError(
                e.message!!.replace("expected:", "expected property ${formatName()} to be")
            )
        }
 ...
 * `it::isIntegerLiteral shouldBe true`. For properties synthesized from Java getters starting with
 * "get", you have to use the name of the getter instead of that of the generated property (with the
 * get prefix).
 *
...
infix fun <T> KCallable<T>.shouldMatch(expected: T.() -> Unit) =
    assertWrapper(this, expected) { n, v -> n should v }
...
fun assertSuppressed(report: Report, size: Int): List<Report.SuppressedViolation> {
    assertEquals(
        size,
        report.suppressedViolations.size,
        message = "Wrong number of suppressed violations!",
    )
    return report.suppressedViolations
}

/**
 * Checks the coordinates of this node. Note that this tests the report location which might not be
