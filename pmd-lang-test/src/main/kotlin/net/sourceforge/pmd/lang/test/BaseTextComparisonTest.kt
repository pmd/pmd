...
/**
 * Compare a dump of a file against a saved baseline. See subclasses CpdTextComparisonTest,
 * BaseTreeDumpTest.
 */
abstract class BaseTextComparisonTest {
    ...
    /**
     * Resource prefix to look for test files. This is resolved from the [resourceLoader] class.
     * Separate directories with '/', not '.'.
     */
    ...
    /**
     * Executes the test. The test files are looked up using the [resourceLoader]. The reference
     * test file must be named [fileBaseName] + [EXPECTED_EXTENSION]. The source file to parse must
     * be named [fileBaseName] + [extensionIncludingDot].
     *
    ...
    internal fun doTest(
        ...
        val expectedFile =
            findTestFile(
                    resourceLoader,
                    "${resourcePrefix}/$fileBaseName$expectedSuffix$EXPECTED_EXTENSION",
                )
                .toFile()
        ...
        assertEquals(
            expected.normalize(),
            actual.normalize(),
            "File comparison failed, see the reference: $expectedFile",
        )
    }

    protected fun sourceText(fileBaseName: String): FileData {
        val sourceFile =
            findTestFile(resourceLoader, "${resourcePrefix}/$fileBaseName$extensionIncludingDot")
                .toFile()

        assert(sourceFile.isFile) { "Source file $sourceFile is missing" }
    ...
    protected open fun String.normalize() =
        replace(
            // \R on java 8+
            regex = Regex("\\u000D\\u000A|[\\u000A\\u000B\\u000C\\u000D\\u0085\\u2028\\u2029]"),
            replacement = "\n",
    ...
    private val srcTestResources = let {
        ...
        System.getProperty("mvn.project.src.test.resources")?.let { Paths.get(it).toAbsolutePath() }
