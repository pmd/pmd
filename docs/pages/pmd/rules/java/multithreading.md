...
keywords: Multithreading, AvoidSynchronizedAtMethodLevel, AvoidThreadGroup, AvoidUsingVolatile, DoNotUseThreads, DontCallThreadRun, DoubleCheckedLocking, NonThreadSafeSingleton, UnsynchronizedStaticDateFormatter, UnsynchronizedStaticMessageDigest, UnsynchronizedStaticFormatter, UseConcurrentHashMap, UseNotifyAllInsteadOfNotify
language: Java
---
...
## UnsynchronizedStaticMessageDigest

**Since:** PMD 6.18.0

**Priority:** Medium (3)

MessageDigest::digest is not synchronized. We should use separate format instances
for each thread. If multiple threads must access a static formatter, the formatter must be
synchronized on block level.

**This rule is defined by the following Java class:** [net.sourceforge.pmd.lang.java.rule.multithreading.UnsynchronizedStaticMessageDigestRule](https://github.com/pmd/pmd/blob/master/pmd-java/src/main/java/net/sourceforge/pmd/lang/java/rule/multithreading/UnsynchronizedStaticMessageDigestRule.java)

**Example(s):**

``` java
{%raw%}public class Foo {
    private static final MessageDigest INSTANCE = MessageDigest.getInstance("SHA-512");
    void bar() {
        byte[] input = new byte[13];
        INSTANCE.digest(input); // poor, no thread-safety
    }
    void foo() {
        byte[] input = new byte[13];
        synchronized (sdf) {
            INSTANCE.digest(input); // preferred
        }
    }
}{%endraw%}
```
...
**Use this rule with the default properties by just referencing it:**
``` xml
<rule ref="category/java/multithreading.xml/UnsynchronizedStaticMessageDigest" />
```
