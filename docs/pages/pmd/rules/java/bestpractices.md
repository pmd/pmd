...
**This rule is defined by the following XPath expression:**
``` xpath
//FieldDeclaration/Type/ReferenceType/ClassOrInterfaceType[@Image = 'MessageDigest']
```
...
``` java
{%raw%}public class Foo {
    private final MessageDigest sharedMd;    // potential problem in unsynchronized method
    ...
    // we expect same result for 100 calculateHashShared(DATA1) calls
    // but we could find different hashes in hash1Futures or hash2Futures
    }
}{%endraw%}
```
