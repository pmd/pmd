...
/**
 * Resolves the auxiliary classpath entries for Kotlin analysis.
 *
 * <p>Resolution order:
 * <ol>
...
final class KotlinAuxClasspathResolver {
    ...
    List<File> resolve() {
        ...
        if (raw != null && !raw.isEmpty()) {
            ...
            LOG.debug("Kotlin aux classpath from string property ({} entries)", entries.size());
        ...
        // TODO(pmd/pmd#4953, pmd/pmd#6312): getAnalysisClassLoader() is deprecated in PMD 7.25.0.
        //   Replace with bundle.getClasspathConfig().open() -> OpenClasspath, close it in
        //   KotlinLanguageProcessor.close(). See JavaLanguageProcessor for the updated pattern.
        ...
        if (!urlEntries.isEmpty()) {
            LOG.debug("Kotlin aux classpath from URLClassLoader hierarchy ({} entries)", urlEntries.size());
        ...
        if (javaClassPath != null && !javaClassPath.isEmpty()) {
            ...
            LOG.debug("Kotlin aux classpath from java.class.path ({} entries)", entries.size());
