...
exports.pmdExecutor = exports.PmdExecutor = void 0;
exports.detectPmdPath = detectPmdPath;
...
const log = {
    ...
    debug: (msg) => {
        if (process.env.DEBUG)
            console.error(`[pmd-mcp] DEBUG: ${msg}`);
    },
};
/**
 * Detect PMD installation path
 */
async function detectPmdPath() {
...
class PmdExecutor {
     ...
     */
    parseCpdXmlOutput(output) {
        const duplications = [];
        let filesAnalyzed = 0;
        try {
            // Simple XML parsing for CPD output
            const duplicationMatches = output.matchAll(/<duplication lines="(\d+)" tokens="(\d+)">([\s\S]*?)<\/duplication>/g);
            for (const match of duplicationMatches) {
                const lines = parseInt(match[1], 10);
                const tokens = parseInt(match[2], 10);
                const content = match[3];
                const files = [];
                const fileMatches = content.matchAll(/<file\s+line="(\d+)"\s+(?:endline="(\d+)"\s+)?(?:column="\d+"\s+)?(?:endcolumn="\d+"\s+)?path="([^"]+)"/g);
...
exports.PmdExecutor = PmdExecutor;
// Default executor instance
exports.pmdExecutor = new PmdExecutor();
//# sourceMappingURL=pmd-executor.js.map