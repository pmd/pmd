            ...
            inContext(ExpressionParsingCtx) {
                chain should
                    parseAs {
                        methodCall("collect") {
                            it shouldHaveType
                                with(it.typeDsl) { gen.t_List[int.box()] } // List<Integer>
                                ...
                                methodCall("peek") {
                                    ...
                                    it::getQualifier shouldBe
                                        methodCall("of") {
                                            ...
                                            it::getArguments shouldBe child { int(1) }
                                        }
            ...
            inContext(ExpressionParsingCtx) {
                chain should
                    parseAs {
                        methodCall("collect") {
                            it shouldHaveType
                                with(it.typeDsl) { gen.t_List[ts.UNKNOWN] } // List</*unresolved*/>

                            it::getQualifier shouldBe
                                methodCall("map") {
                                    it shouldHaveType
                                        with(it.typeDsl) {
                                            gen.t_Stream[ts.UNKNOWN]
                                        } // Stream</*unresolved*/>
                ...
                methodCall("f") {
                    ...
                    argList {
                        exprLambda {
                            lambdaFormals(1)
                            methodCall("length") {
                                variableAccess("s") { it shouldHaveType it.typeSystem.STRING }
                                argList(0)
                            }
                        }
                    }
                    ...
                    argList {
                        exprLambda {
                            ...
                            fieldAccess("i") {
                                ...
                                constructorCall {
                                    variableAccess("s") { it shouldHaveType t_Scratch }
                                    classType("WithField") { it shouldHaveType t_WithField }
                                    argList(0)
                                }
                            }
                        }
...
class Scratch {
    ...
    static {
        ...
        """
            ...
            val (a, b, c, d, e, f, h) =
                acu.descendants(ASTLambdaExpression::class.java).toList {
                    mirrors.getTopLevelFunctionalMirror(it) as ExprMirror.LambdaExprMirror
                }

            fun ExprMirror.LambdaExprMirror.shouldBeCompat(
                void: Boolean = false,
                value: Boolean = false,
            ) {
                withClue(this) {
        ...
        parserTest("Test void compatible lambda with value compatible body") {
            ...
            spy.shouldBeOk { lambda shouldHaveType java.lang.Runnable::class.raw }
        }
        ...
        parserTest("Test void compatible lambda with void body") {
            ...
            spy.shouldBeOk { lambda shouldHaveType DoubleConsumer::class.raw }
        }
        ...
        parserTest("Test explicitly typed lambda with zero params, ground-target type inference") {
            // note that this is not actually implemented, we just special-case the case "zero
            // parameters"
            ...
            spy.shouldBeOk { lambda shouldHaveType Supplier::class[gen.t_String] }
        }
    ...
    interface Foreachable<T> {
        ...
        static <E> Foreachable<E> flatmapForeach(Foreachable<Foreachable<E>> f0) {
            ...
            val (action, f1, f2) =
                acu.descendants(ASTLambdaParameter::class.java).crossFindBoundaries().toList()
...
class NodeStream<T> {
    ...
    public static <Q, V> Iterator<V> mapNotNull(Iterator<? extends Q> it, Function<? super Q, ? extends V> mapper) { return null; }
        ...
        parserTestContainer("Lambda bug with nested lambdas") {
            ...
            doTest("Outside in") { makeTest(false) }

            doTest("Inside out") { makeTest(true) }
        }
            ...
            spy.shouldBeOk {
                ...
                lambda.expressionBody!!.shouldBeA<ASTMethodCall> {
                    it.methodType.shouldMatchMethod(
                        named = "apply",
                        withFormals =
                            listOf(captureMatcher(`?` `super` t_Iterator[`?` extends tvar])),
                    )
