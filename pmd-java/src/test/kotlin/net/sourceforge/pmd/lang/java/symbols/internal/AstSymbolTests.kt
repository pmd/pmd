...
class AstSymbolTests :
    ParserTestSpec({
        ...
        """
            ...
            val (fooClass, innerItf, innerEnum, innerClass, annot) =
                acu.descendants(ASTTypeDeclaration::class.java).crossFindBoundaries().toList {
                    it.symbol
                }

            val (barM, ohioM) =
                acu.descendants(ASTMethodDeclaration::class.java).toList { it.symbol }
            ...
            doTest("should reflect their modifiers") {
                ...
                innerItf::getModifiers shouldBe
                    (Modifier.PRIVATE or Modifier.ABSTRACT or Modifier.STATIC)
            ...
            doTest("(annotations) should have j.l.Annotation as super interface") {
                val annotSym =
                    annot.typeSystem.getClassSymbol(java.lang.annotation.Annotation::class.java)!!
                annot.superInterfaces shouldBe listOf(annotSym)
                annot.getSuperInterfaceTypes(Substitution.EMPTY) shouldBe
                    listOf(annot.typeSystem.declaration(annotSym))
            }
        }
        ...
        """
            ...
            doTest("should reflect their modifiers") {
                ...
                (locals - barLoc - ctorLoc).forEach { it::getModifiers shouldBe 0 }
            }
            ...
            doTest("should have no canonical name") {
                locals.forEach { it::getCanonicalName shouldBe null }
        ...
        """
            ...
            val (pointRecord, point2Record) =
                acu.descendants(ASTRecordDeclaration::class.java).toList { it.symbol }
            val (canonCtor1, canonCtor2) =
                acu.descendants(ASTRecordComponentList::class.java).toList { it.symbol }
            val (auxCtor) =
            ...
            val (xComp, yComp, _, y2Comp, _) =
                acu.descendants(ASTVariableId::class.java).toList { it.symbol }
            ...
            doTest("should reflect their superclass properly") {
                pointRecord.superclass.shouldBeA<JClassSymbol> {
                    it::getCanonicalName shouldBe
                        "java.lang.Record" // even if unresolved bc of host jdk
                }
                pointRecord::getSimpleName shouldBe "Point"
                point2Record::getSimpleName shouldBe "Point2"
            }
        ...
        """
            ...
            doTest("should use the empty string as their simple name") {
                allAnons.forEach { it::getSimpleName shouldBe "" }
            }

            doTest("should have no canonical name") {
                allAnons.forEach { it::getCanonicalName shouldBe null }
                anonMember::getCanonicalName shouldBe null
            }

            doTest("should reflect their enclosing type") {
                (allAnons - anonAnon).forEach { it::getEnclosingClass shouldBe fooClass }
            ...
            doTest("should reflect their superclass") {
                ...
                (allAnons - anonsWithSuperClass.toSet()).forEach {
                    ...
                    it::getSuperInterfaces shouldBe
                        listOf(it.typeSystem.getClassSymbol(Runnable::class.java))
                }
            }
