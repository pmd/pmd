        ...
        with(TypeDslOf(ts)) {
            with(gen) {
                test("Test primitive subtyping") {
                    ...
                    (ts.allPrimitives.toList() - boolean).forEach { it shouldBeUnrelatedTo boolean }
                }

                test("Test primitive arrays are unrelated") {
                    val unArrays = ts.allPrimitives.map { ts.arrayType(it) }
                    ...
                    unArrays.forEach { arr ->
                        ...
                        (unArrays - arr).forEach { arr shouldBeUnrelatedTo it }
                    }
                }
                ...
                test("Test enum f-bound") {
                    ...
                    val sup =
                        ts.parameterise(
                            ts.getClassSymbol(java.lang.Enum::class.java)!!,
                            listOf(someEnum),
                        )
                ...
                test(
                    "Test raw type is convertible to wildcard parameterized type without unchecked conversion"
                ) {
                ...
                test("Test containment") {
                    checkAll(ts.refTypeGen, ts.refTypeGen) { t, s ->
                        if (
                            t != s
                        ) // remember, t and s are types, never wildcards with this generator
                ...
                test("Null type is only compatible with reference types") {
                    forAll(ts.refTypeGen) { ts.NULL_TYPE.isSubtypeOf(it) }

                    ts.allPrimitives.forNone { assertTrue(ts.NULL_TYPE.isSubtypeOf(it)) }
                }

                test("Error type is compatible with anything") {
                    forAll(ts.allTypesGen) { ts.ERROR.isSubtypeOf(it) }
                }

                test("Unresolved type is compatible with anything") {
                    forAll(ts.allTypesGen) { ts.UNKNOWN.isSubtypeOf(it) }
                }

                test("Unresolved symbol is compatible with any class/interface") {
                    val t =
                        ts.declaration(
                                UnresolvedClassStore(ts).makeUnresolvedReference("obj.foo", 0)
                            )
                            .shouldBeUnresolvedClass("obj.foo")

                    checkAll(ts.primitiveGen) { s -> t shouldNotBeSubtypeOf s }

                    checkAll(ts.refTypeGen) { s ->
                        if (s.isArray) t shouldNotBeSubtypeOf s else t shouldBeSubtypeOf s
                    }
                }
                ...
                test("Captured subtyping tests") {
                    checkAll(ts.refTypeGen) { t ->
                        withClue("Upper captures") {
                            capture(t_List[`?` extends t]) shouldSubtypeNoCapture
                                t_List[`?` extends t]
                            t_List[`?` extends t] shouldNotSubtypeNoCapture
                                capture(t_List[`?` extends t])
                        }

                        withClue("Lower captures") {
                            capture(t_List[`?` `super` t]) shouldSubtypeNoCapture
                                t_List[`?` `super` t]
                            t_List[`?` `super` t] shouldNotSubtypeNoCapture
                                capture(t_List[`?` `super` t])
                        }
                    }
                }
