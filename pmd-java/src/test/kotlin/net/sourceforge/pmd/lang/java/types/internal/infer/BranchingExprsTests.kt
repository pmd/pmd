...
class BranchingExprsTests :
    ProcessorTestSpec({
        fun TypeSystem.stringSupplier(): JTypeMirror =
            with(TypeDslOf(this)) { java.util.function.Supplier::class[gen.t_String] }

        parserTestContainer("Test ternary lets context flow") {
            asIfIn(TypeInferenceTestCases::class.java)

            inContext(ExpressionParsingCtx) {
                "makeThree(true ? () -> \"foo\" : () -> \"bar\")" should
                    parseAs {
        ...
        parserTestContainer("Test ternary without context lubs params") {
            ...
            inContext(StatementParsingCtx) {
                "var ter = true ? new ArrayList<String>() : new LinkedList<String>();" should
                    parseAs {
                        localVarDecl {
                            modifiers {}
                            ...
                            variableDeclarator("ter") {
                                val lubOfBothLists =
                                    with(it.typeDsl) {
                                        ts.lub(
                                            gen.`t_ArrayList{String}`,
                                            gen.`t_LinkedList{String}`,
                                        )
                                    }
...
class Scratch {
    ...
    static void putBoolean(byte[] b, int off, boolean val) {
        ...
        parserTest("Cast context doesn't provide target type (only for lambdas)") {
            ...
            val (ternary1, ternary2) =
                acu.descendants(ASTConditionalExpression::class.java).toList()

            spy.shouldBeOk {
                ternary1 shouldHaveType gen.t_Collection[ts.OBJECT] // java.util.Collection<Object>
                ternary2 shouldHaveType
                    gen.`t_Collection{String}` // java.util.Collection<java.lang.String>
            }
        }
        ...
        parserTestContainer("Assignment context doesn't influence standalone ternary") {
            inContext(StatementParsingCtx) {
                ...
                "double ter = true ? new Integer(2) : 3;" should
                    parseAs {
                        localVarDecl {
                            modifiers {}
                            primitiveType(DOUBLE)
        ...
        parserTestContainer("Reference ternary with context has type of its target") {
            inContext(StatementParsingCtx) {
                "Object ter = true ? String.valueOf(1) : String.valueOf(2);" should
                    parseAs {
                        localVarDecl {
                            modifiers {}
                            classType("Object")
                ...
                "String ter = (String) (Object) (true ? String.valueOf(1) : 2);" should
                    parseAs {
                        localVarDecl {
                            modifiers {}
                            classType("String")
