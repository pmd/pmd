...
/**  */
class UnresolvedTypesRecoveryTest :
    ProcessorTestSpec({
...
class C {
    ...
    static {
        ...
        id(new Unresolved());
            ...
            val t_Unresolved =
                acu.descendants(ASTConstructorCall::class.java).firstOrThrow().typeNode.typeMirror
                    as JClassType
            ...
            call.shouldMatchN {
                methodCall("id") {
                    it.methodType.shouldMatchMethod(
                        "id",
                        withFormals = listOf(t_Unresolved),
                        returning = t_Unresolved,
                    )
...
class C {
    ...
    static {
        Unresolved<String> s = new Unresolved<>();
            ...
            val t_UnresolvedOfString =
                acu.descendants(ASTClassType::class.java)
                    .first { it.simpleName == "Unresolved" }!!
                    .typeMirror
                    .shouldBeA<JClassType> {
...
class C {
    ...
    static {
        ...
        String k = SOMETHING;
            ...
            val (field, unqual) =
                acu.descendants(ASTAssignableExpr.ASTNamedReferenceExpr::class.java).toList()
    ...
    void foo(int i) {
        ...
        parserTest("Wrong syntax, return with expr in void method") {
            ...
            for (vaccess in acu.descendants(ASTVariableAccess::class.java)) {
                spy.shouldBeOk { vaccess shouldHaveType ts.UNKNOWN }
            }
        }
        ...
        parserTest("Type inference should not resolve UNKNOWN bounded types to Object #5329") {
            ...
            acu.withTypeDsl {
                collect shouldHaveType
                    java.util.Map::class[ts.UNKNOWN, java.util.List::class[itemT]]
                buildItem.methodType.symbol shouldBe buildItemDecl
            }
        }

        parserTest("Unresolved type should also allow unchecked conversion") {
            // The problem here is that ConstraintViolation<?> is not convertible to
            // ConstraintViolation,
            ...
            val constraintViolationT =
                acu.descendants(ASTClassType::class.java)
                    .filter { it.simpleName == "ConstraintViolation" }
                    .firstOrThrow()
                    .typeMirror
                    .symbol as JClassSymbol
