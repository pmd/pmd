...
class ASTPatternTest :
    ProcessorTestSpec({
        val typePatternsVersions = JavaVersion.since(J16)

        parserTestContainer(
            "Test patterns only available on JDK16 or higher (including preview)",
            javaVersions = JavaVersion.except(typePatternsVersions),
        ) {
            inContext(ExpressionParsingCtx) { "obj instanceof Class c" shouldNot parse() }
        }
        ...
        parserTestContainer("Test simple patterns", javaVersions = typePatternsVersions) {
            ...
            inContext(ExpressionParsingCtx) {
                            ...
                            child<ASTPatternExpression> {
                                it.pattern shouldBe
                                    child<ASTTypePattern> {
                                        it.isAnnotationPresent("java.lang.Deprecated") shouldBe
                                            false
                                        it.modifiers shouldBe
                                            modifiers {} // dummy/empty modifier list
                                        it.typeNode shouldBe classType("Class")
                                        it.varId shouldBe
                                            variableId("c") {
                                                it.hasExplicitModifiers(JModifier.FINAL) shouldBe
                                                    false
                ...
                "obj instanceof final Class c" should
                    parseAs {
                        infixExpr(BinaryOp.INSTANCEOF) {
                            variableAccess("obj")
                            child<ASTPatternExpression> {
                ...
                "obj instanceof @Deprecated Class c" should
                    parseAs {
                        infixExpr(BinaryOp.INSTANCEOF) {
                            variableAccess("obj")
                            child<ASTPatternExpression> {
                                it.pattern shouldBe
                                    child<ASTTypePattern> {
                                        ...
                                        it.modifiers shouldBe modifiers { annotation("Deprecated") }
                                        it.typeNode shouldBe classType("Class")
                                        it.varId shouldBe
                                            variableId("c") {
                                                it.hasExplicitModifiers(JModifier.FINAL) shouldBe
