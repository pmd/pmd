    ...
    ParserTestSpec({
        ...
        parserTestContainer("Simple lambda expressions", javaVersions = J1_8..Latest) {
            inContext(ExpressionParsingCtx) {
                ...
                "(final int a, @F List<String> b) -> foo()" should
                    parseAs {
                        exprLambda {
                            it::isExplicitlyTyped shouldBe true
                            it::getParameters shouldBe
                                lambdaFormals {
                                    lambdaParam {
                                        it::getModifiers shouldBe
                                            modifiers {
                                                it::getExplicitModifiers shouldBe
                                                    setOf(JModifier.FINAL)
                                            }
        ...
        parserTestContainer("Mixed array notation/varargs", javaVersions = J1_8..Latest) {
            inContext(ExpressionParsingCtx) {
                "(final int a[]@B[], @F List<String>@a [] b @A []) -> foo()" should
                    parseAs {
                        exprLambda {
                            it::isExplicitlyTyped shouldBe true
                            it::getParameters shouldBe
                                    ...
                                    lambdaParam {
                                        ...
                                        it::getTypeNode shouldBe
                                            arrayType {
                                                classType("String")
                                                it::getDimensions shouldBe
                                                    child { varargsArrayDim {} }
                                            }

                                        variableId("b") { it::getExtraDimensions shouldBe null }
                                    }
                                }
                                    ...
                                    lambdaParam {
                                        ...
                                        it::getTypeNode shouldBe
                                            arrayType {
                                                ...
                                                it::getDimensions shouldBe
                                                    child {
                                                        arrayDim { annotation("A") }
                                                        varargsArrayDim { annotation("B") }
                                                    }
                                            }
        ...
        parserTestContainer("Negative lambda contexts") {
            inContext(StatementParsingCtx) { "a -> {}" shouldNot parse() }
        }

        parserTestContainer("Positive lambda contexts") {
            inContext(ExpressionParsingCtx) { "(a -> {})" should parse() }
        }
    })
