    ...
    FunSpec({
        ...
        test("Test primitive types are reused") {
            listOf(true, false).forEach { isErased ->
                ...
                ts.typeOf(ts.getClassSymbol(Character.TYPE), isErased)
                    .shouldBeSameInstanceAs(ts.CHAR)
                ts.typeOf(ts.getClassSymbol(Void.TYPE), isErased).shouldBeSameInstanceAs(ts.NO_TYPE)
                ts.typeOf(ts.getClassSymbol(Void::class.java), isErased)
                    .shouldBeSameInstanceAs(ts.BOXED_VOID)
                ts.typeOf(ts.getClassSymbol(Object::class.java), isErased)
                    .shouldBeSameInstanceAs(ts.OBJECT)
            }
        }
        ...
        test("Test typeOf array") {
            val arraySym =
                ts.getClassSymbol(Cloneable::class.java.arrayType)!!.also {
            ...
            arrayT.shouldBeA<JArrayType> {
                ...
                it.componentType.shouldBeA<JClassType> {
                    it.symbol shouldBe ts.getClassSymbol(Cloneable::class.java)
                }
            }
        }
        ...
        test("Test typeOf array of type var") {
            ...
            type.shouldBeA<JArrayType> { it.componentType shouldBeSameInstanceAs tvar }
        ...
        test("Test isTop") {
            ...
            val javaLangObjectDeclaration =
                javaParser.parseSomeClass("package java.lang; class Object { }")
            javaLangObjectDeclaration::isTop shouldBe true
        }

        test("Test isTop on annotated Object") {
            val annotated =
                ts.OBJECT.addAnnotation(
                    FakeSymAnnot(ts.getClassSymbol(ClassWithTypeAnnotationsInside.A::class.java))
                )
        ...
        test("Test parameterize generic class") {
            ...
            parameterized shouldBe
                (ts.rawType(listSym) as JClassType).withTypeArguments(listOf(ts.INT.box()))
        }

        test("Test array type recursive") {
            for (t in listOf(ts.INT, ts.OBJECT, ts.CLONEABLE)) {
                shouldThrow<java.lang.IllegalArgumentException> { ts.arrayType(t, -1) }
        ...
        test("Test malformed wildcard bound") {
            ...
            shouldThrow<java.lang.IllegalArgumentException> { ts.wildcard(false, ts.INT) }
        }
