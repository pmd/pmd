...
class CaptureTest :
    ...
    FunSpec({
        with(TypeDslOf(testTypeSystem)) {
            with(gen) {
                test("Capture is a noop on raw types") { capture(t_List) shouldBe t_List }

                test("Capture merges declared bounds and bounds of wildcard") {
                    val t_Scratch =
                        javaParser.parseSomeClass("class Scratch<T extends java.util.List<T>> {}")
                ...
                test(
                    "Capture merges declared bounds and bounds of wildcard (self referential bound)"
                ) {
                    val t_Scratch =
                        javaParser.parseSomeClass("class Scratch<T extends Scratch<T>> {}")
                ...
                test("Capture of malformed types") {
                    ...
                    val matcher =
                        captureMatcher(`?` extends t_String).also {
