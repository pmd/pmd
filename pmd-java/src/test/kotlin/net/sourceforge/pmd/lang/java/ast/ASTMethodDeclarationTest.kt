...
class ASTMethodDeclarationTest :
    ParserTestSpec({
        parserTestContainer(
            "Non-private interfaces members should be public",
            javaVersions = Earliest..Latest,
        ) {
        ...
        parserTestContainer("Private methods in interface should be private", J9..Latest) {
            inContext(TypeBodyParsingCtx) {
                "private int de() { return 1; }" should
                    parseAs {
                        methodDecl {
        ...
        parserTestContainer(
            "Non-default methods in interfaces should be abstract",
            javaVersions = J1_8..Latest,
        ) {
            genClassHeader = "interface Bar"
        ...
        parserTestContainer(
            "Default methods in interfaces should not be abstract",
            javaVersions = J1_8..Latest,
        ) {
            genClassHeader = "interface Bar"

            inContext(TypeBodyParsingCtx) {
                "default int kar() { return 1; } " should
                    parseAs {
                        methodDecl {
            ...
            // default abstract is an invalid combination of modifiers, so we won't encounter it in
            // real analysis
        }

        parserTestContainer("Throws list") {
            inContext(TypeBodyParsingCtx) {
                "void bar() throws IOException, java.io.Bar { }" should
                    parseAs {
                        methodDecl {
                            ...
                            it::getThrowsList shouldBe
                                throwsList {
                                    classType("IOException")
                                    classType("Bar") { ambiguousName("java.io") }
                                }
        ...
        parserTestContainer("Throws list can be annotated") {
            inContext(TypeBodyParsingCtx) {
                "void bar() throws @Oha IOException, @Aha java.io.@Oha Bar { }" should
                    parseAs {
                        methodDecl {
                            it::getModifiers shouldBe modifiers {}
                            ...
                            it::getThrowsList shouldBe
                                throwsList {
                                    classType("IOException") { annotation("Oha") }

                                    classType("Bar") {
                                        classType("io") { classType("java") { annotation("Aha") } }
                                        annotation("Oha")
                                    }
                                }
        ...
        parserTestContainer("Varargs can be annotated") {
            inContext(TypeBodyParsingCtx) {
                "void bar(@Oha IOException @Aha ... java) { }" should
                    parseAs {
                        methodDecl {
                            it::getModifiers shouldBe modifiers {}
                            ...
                            it::getFormalParameters shouldBe
                                formalsList(1) {
                                    child<ASTFormalParameter> {
                                        ...
                                        it::getModifiers shouldBe modifiers { annotation("Oha") }
                                        arrayType {
                                            classType("IOException")
                                            it::getDimensions shouldBe
                                                child { varargsArrayDim { annotation("Aha") } }
                                        }
                ...
                "void bar(@Oha IOException []@O[] @Aha ... java) { }" should
                    parseAs {
                        methodDecl {
                            it::getModifiers shouldBe modifiers {}
                                    ...
                                    child<ASTFormalParameter> {
                                        ...
                                        arrayType {
                                            ...
                                            it::getDimensions shouldBe
                                                child {
                                                    arrayDim {}
                                                    arrayDim { annotation("O") }
                                                    varargsArrayDim { annotation("Aha") }
                                                }
                                        }
                                        variableId("java")
                                    }
                ...
                "int bar() @NonZero [];" should
                    parseAs {
                        annotationMethod {
                            ...
                            it::getExtraDimensions shouldBe
                                child { arrayDim { annotation("NonZero") } }
                        }
                    }

                "Override bar() default @Override;" should
                    parseAs {
                        annotationMethod {
                            it::getModifiers shouldBe modifiers {}
                ...
                "Override bar()[] default { @Override };" should
                    parseAs {
                        annotationMethod {
                            ...
                            it::getDefaultClause shouldBe
                                defaultValue { memberValueArray { annotation("Override") } }
                        }
                    }
            }
        }
        ...
        parserTestContainer("Receiver parameters") {
            ...
            */
            ...
            inContext(TypeBodyParsingCtx) {
                ...
                "void bar(@A Foo this, int other);" should
                    parseAs {
                        methodDecl {
                            it::isAbstract shouldBe false
                            it::getName shouldBe "bar"
        ...
        parserTestContainer("Annotation placement") {
            inContext(TypeBodyParsingCtx) {
                "@OnDecl <T extends K> @OnType Ret bar() { return; }" should
                    parseAs {
                        methodDecl {
                            it::getName shouldBe "bar"
                            ...
                            it::getResultTypeNode shouldBe classType("Ret") { annotation("OnType") }
