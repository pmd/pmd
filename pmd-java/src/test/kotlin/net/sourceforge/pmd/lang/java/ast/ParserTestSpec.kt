...
/**
 * Base class for grammar tests that use the DSL. Tests are layered into containers that make it
 * easier to browse in the IDE. Layout is group name, then java version, then test case. Test cases
 * are "should" assertions matching a string against a matcher defined in [ParserTestCtx] or
 * explicitly defined test cases with "doTest".
 *
 * <p>If only one explicit test is written, then use "parserTest" as the grouping and just put in
 * assertions. A single test case will be defined implicitly then.
 * <pre>{@code
 ...
 */
abstract class ParserTestSpec(body: ParserTestSpec.() -> Unit) :
    DslDrivenSpec(), RootScope, IntelliMarker {
    ...
    /**
     * Defines a group of tests that should be named similarly, with separate tests for separate
     * versions.
     *
     * Calls to "should" in the block are intercepted to create a new test, with the given [name] as
     * a common prefix.
     *
     * This is useful to make a batch of grammar specs for grammar regression tests without
     * bothering to find a name.
     *
     * @param name Name of the container test
     * @param spec Assertions. Each call to [io.kotest.matchers.should] on a string receiver is
     *   replaced by a [GroupTestCtx.should], which creates a new parser test.
     */
    private fun parserTestGroup(
        name: String,
    ...
    /**
     * Defines a single test case, that is executed on several java versions and grouped by the
     * given [name].
     *
     * @param name Name of the container test
                ...
                doTest(name) {
                    ...
                    if (this@onVersions.hasMoreThanOneChild()) {
                        throw IllegalStateException(
                            "Expected no child test cases, but parserTest '$name' has children. Use 'parserTestContainer' instead."
                        )
                    }
                }
            }
    ...
    /**
     * Defines a group of tests that should be named similarly, executed on several java versions.
     * Calls to "should" in the block are intercepted to create a new test, with the given [name] as
     * a common prefix.
     *
     * Alternatively you can use "doTest" to define a new test case explicitly.
     *
     * This is useful to make a batch of grammar specs for grammar regression tests without
     * bothering to find a name.
     *
     ...
     * @param spec Assertions. Each call to [io.kotest.matchers.should] on a string receiver is
     *   replaced by a [GroupTestCtx.VersionedTestCtx.should], which creates a new parser test case.
     *   Alternatively use [GroupTestCtx.VersionedTestCtx.doTest] to create a new parser test case
     *   explicitly.
     */
    fun parserTestContainer(
        name: String,
        ...
        parserTestGroup(name) {
            onVersions(javaVersions) {
                ...
                if (!this@onVersions.hasChildren()) {
                    throw IllegalStateException(
                        "Expected at least one child test case, but parserTestContainer '$name' has no children. Use 'parserTest' instead or use 'should'/'doTest'."
                    )
                }
            }
        }
    ...
    /**
     * Setup to apply to spawned [ParserTestCtx]. By default, AST processing is disabled beyond the
     * parser.
     */
    ...
    inner class GroupTestCtx(testScope: TestScope) : AbstractContainerScope(testScope) {
            ...
            javaVersions.forEach { javaVersion ->
                val nested =
                    NestedTest(
                        ...
                        test = {
                            this@GroupTestCtx.VersionedTestCtx(this, javaVersion)
                                .apply { setup() }
                                .spec()
                        },
                        config = null,
        ...
        inner class VersionedTestCtx(testScope: TestScope, javaVersion: JavaVersion) :
            ParserTestCtx(testScope, javaVersion) {

            suspend fun doTest(
                name: String = "[unnamed test case]",
                assertions: suspend VersionedTestCtx.() -> Unit,
            ) {
            ...
            suspend infix fun String.should(matcher: Assertions<String>) {
                containedParserTestImpl(
                    this@VersionedTestCtx,
                    "'$this'",
                    javaVersion = javaVersion,
                ) {
            ...
            suspend infix fun String.shouldNot(matcher: Matcher<String>) = should(matcher.invert())

            suspend fun <T : Node> inContext(
                nodeParsingCtx: NodeParsingCtx<T>,
                assertions: suspend ImplicitNodeParsingCtx<T>.() -> Unit,
            ) {
                ImplicitNodeParsingCtx(nodeParsingCtx).assertions()
            }

            inner class ImplicitNodeParsingCtx<T : Node>(
                private val nodeParsingCtx: NodeParsingCtx<T>
            ) {
                ...
                fun doParse(s: String): T = nodeParsingCtx.parseNode(s, this@VersionedTestCtx)
                ...
                fun throwParseException(
                    expected: (ParseException) -> Unit = {}
                ): Assertions<String> = this@VersionedTestCtx.notParseIn(nodeParsingCtx, expected)

                fun parseAs(matcher: ValuedNodeSpec<Node, out Any>): Assertions<String> = { str ->
                    nodeParsingCtx.parseNode(str, this@VersionedTestCtx).shouldMatchN(matcher)
                }
            }
        }
    }
