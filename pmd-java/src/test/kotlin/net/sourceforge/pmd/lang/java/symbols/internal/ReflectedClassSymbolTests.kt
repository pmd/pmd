 ...
 * @since 7.0.0
 */
class ReflectedClassSymbolTests :
    IntelliMarker,
    WordSpec({
        "A class symbol" should
            {
                "reflect its superclass correctly" {
                    TestClassesGen.forAllEqual {
                        ...
                        else classSym(it)!!.superclass to classSym(it.superclass)
                    }
                }
                ...
                "reflect its type parameters correctly" {
                    TestClassesGen.checkAll { clazz ->
                        ...
                        classSym.typeParameters.map { it!!.name } shouldBe
                            clazz.typeParameters.toList().map { it.name }
                ...
                "reflect its superinterfaces correctly" {
                    TestClassesGen.forAllEqual {
                        classSym(it)!!.superInterfaces to
                            it.interfaces.toList().map { clazz -> classSym(clazz) }
                    }
                }

                "reflect its simple name properly" {
                    TestClassesGen.forAllEqual { classSym(it)!!.simpleName to it.simpleName }
                }

                "reflect its canonical name properly" {
                    TestClassesGen.forAllEqual { classSym(it)!!.canonicalName to it.canonicalName }
                }

                "reflect its modifiers properly" {
                    TestClassesGen.forAllEqual { classSym(it)!!.modifiers to it.modifiers }
                }
