 ...
 * @since 7.0.0
 */
class ASTAssignmentExpressionTest :
    ParserTestSpec({
        parserTestContainer("Simple assignment expressions") {
            inContext(ExpressionParsingCtx) {
                "a = b -> { foo(b); }" should
                    parseAs {
                        assignmentExpr(ASSIGN) {
                            ...
                            it::getRightOperand shouldBe
                                child<ASTLambdaExpression> { unspecifiedChildren(2) }
                        }
                    }
                ...
                "a %= b" should parseAs { assignmentExpr(MOD_ASSIGN) }

                "a /= b" should parseAs { assignmentExpr(DIV_ASSIGN) }

                "a &= b" should parseAs { assignmentExpr(AND_ASSIGN) }

                "a |= b" should parseAs { assignmentExpr(OR_ASSIGN) }

                "a ^= b" should parseAs { assignmentExpr(XOR_ASSIGN) }

                "a += b" should parseAs { assignmentExpr(ADD_ASSIGN) }

                "a -= b" should parseAs { assignmentExpr(SUB_ASSIGN) }

                "a <<= b" should parseAs { assignmentExpr(LEFT_SHIFT_ASSIGN) }

                "a >>= b" should parseAs { assignmentExpr(RIGHT_SHIFT_ASSIGN) }

                "a >>>= b" should parseAs { assignmentExpr(UNSIGNED_RIGHT_SHIFT_ASSIGN) }
            }
        }
