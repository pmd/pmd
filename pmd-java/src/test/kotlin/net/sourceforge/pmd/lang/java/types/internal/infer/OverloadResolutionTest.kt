...
class OverloadResolutionTest :
    ProcessorTestSpec({
        ...
        test("Test conversion in compat tests") {
            fun assertConvertible(
                types: Pair<JTypeMirror, JTypeMirror>,
                pos: Boolean,
                canBox: Boolean = true,
            ) {
                val (t, s) = types
            ...
            fun assertNotConvertible(
                types: Pair<JTypeMirror, JTypeMirror>,
                canBox: Boolean = true,
            ) {
                if (types.first != types.second) assertConvertible(types, false, canBox)
            }
                ...
                "of($t_SomeEnum_name.FOO)" should
                    parseAs {
                        methodCall("of") {
                            ...
                            it::getArguments shouldBe child { fieldAccess("FOO") }
                        }
                    }
            }
        }
        ...
        parserTestContainer("Test partially unresolved") {
            ...
            inContext(ExpressionParsingCtx) {
                "of(DoesntExist.FOO)" should
                    parseAs {
                        methodCall("of") {
                            with(it.typeDsl) {
                                // EnumSet</*unresolved*/>
                                it shouldHaveType gen.t_EnumSet[ts.UNKNOWN]
                                // Overloads::of(/*unresolved*/) ->
                                // java.util.EnumSet</*unresolved*/>
                                it.methodType.shouldMatchMethod(
                                    named = "of",
                                    declaredIn = typeOf(t_Overloads_name),
        ...
        parserTestContainer("Test strict overload 2 args") {
            ...
            inContext(ExpressionParsingCtx) {
                "of($t_SomeEnum_name.FOO, $t_SomeEnum_name.BAR)" should
                    parseAs {
                        methodCall("of") {
                            with(it.typeDsl) {
                                val t_SomeEnum = typeOf(t_SomeEnum_name)
        ...
        parserTestContainer("Test varargs overload") {
            ...
            inContext(ExpressionParsingCtx) {
                "of($t_SomeEnum_name.FOO, $t_SomeEnum_name.BAR,  $t_SomeEnum_name.BAR)" should
                    parseAs {
                        methodCall("of") {
                            with(it.typeDsl) {
                                val t_SomeEnum = typeOf(t_SomeEnum_name)
        ...
        parserTestContainer("Test varargs overload with array") {
            ...
            inContext(ExpressionParsingCtx) {
                "of($t_SomeEnum_name.FOO, new $t_SomeEnum_name[]{$t_SomeEnum_name.BAR,  $t_SomeEnum_name.BAR})" should
                    parseAs {
                        methodCall("of") {
                            // SomeEnum
                            ...
                            with(it.typeDsl) {
                                ...
                                it.overloadSelectionInfo::isVarargsCall shouldBe
                                    false // selected in strict phase
                                // Overloads::of(SomeEnum, SomeEnum...) -> EnumSet<SomeEnum>
                                it.methodType.shouldMatchMethod(
                                    named = "of",
                                    declaredIn = typeOf(t_Overloads_name),
                                    ...
                                    child<ASTArrayAllocation> {
                                        ...
                                        child<ASTArrayInitializer> {
                                            ...
                                            fieldAccess("BAR") {
                                                ...
                                                typeExpr { qualClassType(t_SomeEnum_name) }
                                            }
                                            fieldAccess("BAR") {
                                                it shouldHaveType t_SomeEnum
                                                typeExpr { qualClassType(t_SomeEnum_name) }
                            ...
                            methodCall("map") {
                                ...
                                it::getQualifier shouldBe
                                    methodCall("stream") {
                                        ...
                                        it shouldHaveType
                                            with(it.typeDsl) { gen.t_Stream[Class::class[`?`]] }
                                        unspecifiedChild()
                                        argList {
                                            variableAccess("genArray") {
                                                it shouldHaveType
                                                    with(it.typeDsl) { Class::class[`?`].toArray() }
                                            }
                                        }
                                    }
                                ...
                                argList {
                                    methodRef("getTypeName") {
                                        with(it.typeDsl) {
                                            it shouldHaveType
                                                gen.t_Function[Class::class[`?`], gen.t_String]
                                        }
