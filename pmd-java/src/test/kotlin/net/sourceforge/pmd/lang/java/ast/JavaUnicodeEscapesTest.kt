...
fun makeJavaTranslatedDocument(code: String): TextDocument {
    val base = TextDocument.readOnlyString(code, JavaParsingHelper.DEFAULT.defaultVersion)
    return InternalApiBridge.javaTokenDoc().translate(base)
}

class JavaUnicodeEscapesTest :
    IntelliMarker,
    FunSpec({
        test("Test java invalid unicode escapes") {
            ...
            val exception =
                shouldThrow<MalformedSourceException> { makeJavaTranslatedDocument(comment) }
        ...
        test("Test incomplete unicode escape ") {
            ...
            val mse = shouldThrow<MalformedSourceException> { makeJavaTranslatedDocument(comment) }
