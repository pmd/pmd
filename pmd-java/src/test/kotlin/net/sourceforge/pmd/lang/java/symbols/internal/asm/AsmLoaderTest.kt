...
class AsmLoaderTest :
    ...
    FunSpec({
        ...
        val contextClasspath = Classpath {
            Thread.currentThread().contextClassLoader.getResourceAsStream(it)
        }
        ...
        test("First ever ASM test") {
            ...
            val loaded =
                symLoader.resolveClassFromBinaryName("javasymbols.testdata.StaticNameCollision")!!
        ...
        /**
         * Note: we don't share a single resolver between all tests because that could cause issues
         * because of the order of classloading (the order of test execution).
         */
        ...
        test("Inner names with dollars") {
            ...
            val inner =
                symLoader().resolveClassFromBinaryName("$outerName\$AnInner\$ClassWithDollar")!!

            inner.simpleName shouldBe "AnInner\$ClassWithDollar"
            inner.canonicalName shouldBe
                "javasymbols.testdata.deep.Another\$ClassWith\$Dollar.AnInner\$ClassWithDollar"
        }
        ...
        test("Deeper inner names with dollars") {
            ...
            val symLoader = symLoader()
            val deeper =
                symLoader.resolveClassFromBinaryName(
                    "$outerName\$AnInner\$ClassWithDollar\$ADeeper\$ClassWithDollar"
                )!!

            deeper.simpleName shouldBe "ADeeper\$ClassWithDollar"
            deeper.canonicalName shouldBe
                "javasymbols.testdata.deep.Another\$ClassWith\$Dollar.AnInner\$ClassWithDollar.ADeeper\$ClassWithDollar"

            symLoader.resolveClassFromCanonicalName(deeper.canonicalName!!) shouldBeSameInstanceAs
                deeper
        }
        ...
        test("Enum constants") {
            ...
            val withConstants =
                symLoader().resolveClassFromBinaryName("$outerName\$SomeConstants")!!
