...
enum class JavaVersion : Comparable<JavaVersion> {
    ...
    val pmdName: String =
        name.removePrefix("J").replaceFirst("__", "-").replace('_', '.').lowercase()
    ...
    /**
     * Overloads the range operator, e.g. (`J9..J11`). If both operands are the same, a singleton
     * list is returned.
     */
...
object CustomTreePrinter : KotlintestBeanTreePrinter<Node>(NodeTreeLikeAdapter) {
    ...
    override fun getContextAroundChildAssertion(
        node: Node,
        childIndex: Int,
        actualPropertyAccess: String,
    ): Pair<String, String> {
        val javaGetterName = convertKtPropAccessToGetterAccess(actualPropertyAccess)
...
private val javaImplicitAssertions: Assertions<Node> = {
    ...
    if (it is ASTExpression) run { it::isParenthesized shouldBe (it.parenthesisDepth > 0) }

    if (it is InternalInterfaces.AtLeastOneChild) {
        assert(it.numChildren > 0) { "Expected at least one child for $it" }
    }

    if (it is ModifierOwner)
        run {
            it.modifiers.effectiveModifiers.shouldContainAll(it.modifiers.explicitModifiers)
            it.modifiers.effectiveModifiers.shouldContainAtMostOneOf(
                JModifier.PUBLIC,
                JModifier.PRIVATE,
                JModifier.PROTECTED,
            )
...
/**
 * Extensible environment to describe parse/match testing workflows in a concise way. Can be used
 * inside of a [ParserTestSpec] with [ParserTestSpec.parserTest].
 *
 * Parsing contexts allow to parse a string containing only the node you're interested in instead of
 * writing up a full class that the parser can handle. See [ExpressionParsingCtx],
 * [StatementParsingCtx].
 *
 * These are implicitly used by [matchExpr] and [matchStmt], which specify a matcher directly on the
 * strings, using their type parameter and the info in this test context to parse, find the node,
 * and execute the matcher in a single call. These may be used by [io.kotest.matchers.should], e.g.
 *
 ...
 *
 * Technically the utilities provided by this class may be used outside of
 * [io.kotest.core.spec.Spec]s, e.g. in regular JUnit tests, but I think we should strive to
 * uniformize our testing style, especially since KotlinTest defines so many.
 *
 ...
 * @property genClassHeader Header of the enclosing class used in parsing contexts like
 *   parseExpression, etc. E.g. "class Foo"
 */
open class ParserTestCtx(
    ...
    private var registeredTestCases: Int = 0,
    ...
    /**
     * Will throw on the first semantic error or warning. Useful because it produces a stack trace
    ...
    internal val packageDecl: String
        get() = if (packageName.isEmpty()) "" else "package $packageName;"

    /**
     * Places all node parsing contexts inside the declaration of the given class of the given
     * class. It's like you were writing e.g. expressions inside the class, with the method
     * declarations around it and all.
     *
     * LIMITATIONS:
     * - does not work for [TopLevelTypeDeclarationParsingCtx]
     * - [klass] must be a toplevel class (not an enum, not an interface, not
     *   nested/local/anonymous)
     */
    fun asIfIn(klass: Class<*>) {
        assert(!klass.isArray && !klass.isPrimitive) { "$klass has no class name" }
    ...
    fun notParseIn(
        nodeParsingCtx: NodeParsingCtx<*>,
        expected: (ParseException) -> Unit = {},
    ): Assertions<String> = {
        val e = shouldThrow<ParseException> { nodeParsingCtx.parseNode(it, this) }
        expected(e)
    }

    fun parseIn(nodeParsingCtx: NodeParsingCtx<*>) =
