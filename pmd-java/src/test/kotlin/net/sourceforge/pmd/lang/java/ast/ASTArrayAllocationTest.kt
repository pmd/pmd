    ...
    ParserTestSpec({
        parserTestContainer("Array creation") {
            inContext(ExpressionParsingCtx) {
                ...
                "new @Foo int[3][2]" should
                    parseAs {
                        arrayAlloc {
                            it::getTypeNode shouldBe
                                arrayType({ primitiveType(INT) { annotation("Foo") } }) {
                                    dimExpr { int(3) }
                                    dimExpr { int(2) }
                                }
                        }
                    ...
                    parseAs {
                        arrayAccess {
                            parenthesized {
                                arrayAlloc {
                                    it::getTypeNode shouldBe
                                        arrayType({ primitiveType(INT) }) { dimExpr { int(3) } }
                                }
                            }
                ...
                "new Foo[] { f, g }" should
                    parseAs {
                        arrayAlloc {
                            ...
                            it::getArrayInitializer shouldBe
                                child {
                                    variableAccess("f")
                                    variableAccess("g")
                                }
                ...
                "new int[][] { { 1 }, { 2 } }" should
                    parseAs {
                        arrayAlloc {
                            ...
                            it::getArrayInitializer shouldBe
                                child {
                                    child<ASTArrayInitializer> { int(1) }

                                    child<ASTArrayInitializer> { int(2) }
                                }
                        }
                    }

                "new int[][] { { 1 , 2 }, null }" should
                    parseAs {
                        arrayAlloc {
                            it::getArrayDepth shouldBe 2
