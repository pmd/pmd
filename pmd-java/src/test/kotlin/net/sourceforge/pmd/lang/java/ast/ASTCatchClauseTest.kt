...
class ASTCatchClauseTest :
    ParserTestSpec({
        parserTestContainer("Test crash on multicatch", javaVersions = Earliest..J1_6) {
            inContext(StatementParsingCtx) {
                "try { } catch (IOException | AssertionError e) { }" should
                    throwParseException {
                        it.message.shouldContain(
                            "Composite catch clauses are a feature of Java 1.7, you should select your language version accordingly"
                        )
                    }
            }
        }
        ...
        parserTestContainer("Test single type", javaVersions = J1_5..Latest) {
            ...
            inContext(StatementParsingCtx) {
                "try { } catch (IOException ioe) { }" should
                    parseAs {
                        tryStmt {
        ...
        parserTestContainer("Test annotated multicatch", javaVersions = J1_8..Latest) {
            ...
            inContext(StatementParsingCtx) {
                "try { } catch (@B IOException | @A AssertionError e) { }" should
                    parseAs {
                        tryStmt {
                            it::getBody shouldBe block {}
                            catchClause("e") {
                                catchFormal("e") {
                                    ...
                                    unionType {
                                        classType("IOException")
                                        classType("AssertionError") { annotation("A") }
                                    }
