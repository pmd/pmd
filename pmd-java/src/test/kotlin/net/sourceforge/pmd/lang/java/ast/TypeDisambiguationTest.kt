    ...
    ParserTestSpec({
        ...
        parserTestContainer("Fully qualified names") {
            ...
            inContext(TypeParsingCtx) {
                "javasymbols.testdata.Statics" should
                    parseAs {
                ...
                "javasymbols.testdata.Statics.PublicStatic" should
                    parseAs {
        ...
        """
            ...
            doTest("Unresolved inner type should have a symbol anyway") {
                fooBar.shouldMatchN {
                    classType("Bar") {
                        classType("Foo") { it::getReferencedSym shouldBe foo.symbol }
        ...
        """
            ...
            val (refInFoo, refInScratch) =
                acu.descendants(ASTFieldDeclaration::class.java)
                    .crossFindBoundaries()
                    .map { it.typeNode as ASTClassType }
                    .toList()
            val (_, _, aMem) =
                acu.descendants(ASTClassDeclaration::class.java).crossFindBoundaries().toList {
                    it.symbol
                }

            doTest("Ambiguous inner type should produce an error (ref in Foo)") {
                val (_, args) =
                    logger.warnings[AMBIGUOUS_NAME_REFERENCE]?.first { it.first == refInFoo }!!
                args.map { it.toString() } shouldBe
                    listOf("Mem", "p.Scratch.A.Mem", "p.Scratch.B.Mem")
            }

            doTest("Ambiguous inner type should produce an error (ref in Scratch)") {
                val (_, args) =
                    logger.warnings[AMBIGUOUS_NAME_REFERENCE]?.first { it.first == refInScratch }!!
                args.map { it.toString() } shouldBe
                    listOf("Mem", "p.Scratch.A.Mem", "p.Scratch.B.Mem")
            }
        ...
        """
            ...
            fun assertErrored(t: ASTClassType, expected: Int, actual: Int) {
                val errs =
                    logger.warnings[MALFORMED_GENERIC_TYPE]?.filter { it.first == t } ?: emptyList()
        ...
        """
            ...
            val outerT =
                outerUnresolved.typeMirror.shouldBeA<JClassType> {
            ...
            val innerT =
                m0.typeMirror.shouldBeA<JClassType> {
        ...
        """
            ...
            fun assertErrored(t: ASTClassType) {
                val errs =
                    logger.warnings[EXPECTED_ANNOTATION_TYPE]?.filter { it.first == t }
                        ?: emptyList()
                assertEquals(errs.size, 1, "`${t.text}` should have produced a single error")
        ...
        parserTest(
            "!TODO Import on demand of class defined in same compilation unit that has an extends clause"
        ) {
            // the extends clause must be disambiguated early to process the on demand import (ODI),
            // which is itself needed to resolve a supertype of Foo (which is itself part of the
            // ODI)
