...
class ASTInstanceOfExpressionTest :
    ParserTestSpec({
                    ...
                    parseAs {
                        infixExpr(BinaryOp.INSTANCEOF) {
                            ...
                            it::getRightOperand shouldBe
                                typeExpr { classType("K") { annotation("A") } }
                        }
                    }
            }
        }
        ...
        parserTestContainer("Instanceof with pattern") {
            inContext(ExpressionParsingCtx) {
                "o instanceof String s && s.length() > 4" should
                    parseAs {
        ...
        parserTestContainer(
            "InstanceofExpression cannot test primitive types",
            JavaVersion.except(JavaVersion.J23__PREVIEW, JavaVersion.J24__PREVIEW),
        ) {
            inContext(ExpressionParsingCtx) {
        ...
        parserTestContainer(
            "InstanceofExpression can test primitive types",
            listOf(JavaVersion.J23__PREVIEW, JavaVersion.J24__PREVIEW),
        ) {
            inContext(ExpressionParsingCtx) {
                PrimitiveTypeKind.values().forEach { typeKind ->
                    "f instanceof ${typeKind.simpleName}" should
                        parseAs {
                            infixExpr(BinaryOp.INSTANCEOF) {
                                it::getLeftOperand shouldBe variableAccess("f")
                                it::getRightOperand shouldBe typeExpr { primitiveType(typeKind) }
                            }
                        }

                    "f instanceof @A ${typeKind.simpleName}" should
                        parseAs {
                            infixExpr(BinaryOp.INSTANCEOF) {
                                it::getLeftOperand shouldBe variableAccess("f")
                                it::getRightOperand shouldBe
                                    typeExpr { primitiveType(typeKind) { annotation("A") } }
                            }
                        }
                }
            }
