    ...
    ParserTestSpec({
        parserTestContainer("String literal") {
            inContext(ExpressionParsingCtx) {
                ...
                "\"foo\\t\"" should
                    parseAs { stringLit("\"foo\\t\"") { it::getConstValue shouldBe "foo\t" } }
...
$delim
   ...
   $delim
    ...
    x$delim
        ...
        parserTestContainer(
            "Text block literal on non-JDK13 preview",
            javaVersions = Earliest.rangeTo(J14),
        ) {
            val delim = "\"\"\""
   ...
   winter$delim
        ...
        parserTestContainer("String literal escapes") {
            inContext(ExpressionParsingCtx) {
                "\"abc\u1234abc\"" should
                    parseAs {
                        stringLit("\"abc\u1234abc\"") { it::getConstValue shouldBe "abc\u1234abc" }
                    }
                ...
                "\"abcüabc\"" should
                    parseAs { stringLit("\"abcüabc\"") { it::getConstValue shouldBe "abcüabc" } }
            }
        }
        ...
        parserTestContainer("String literal octal escapes") {
            inContext(ExpressionParsingCtx) {
                ...
                "\"\\123\"" should
                    parseAs {
                        stringLit("\"\\123\"") { it::getConstValue shouldBe char.toString() }
                    }
                val delim = "\"\"\""
                ...
                $delim
            """ should
                    parseAs { textBlock { it::getConstValue shouldBe char.toString() + "\n" } }
            }
        }

        parserTestContainer("Char literal") {
            inContext(ExpressionParsingCtx) {
                "'c'" should parseAs { charLit("'c'") { it::getConstValue shouldBe 'c' } }

                "('c')" should parseAs { charLit("'c'") { it::getConstValue shouldBe 'c' } }

                "'\t'" should parseAs { charLit("'\t'") { it::getConstValue shouldBe '\t' } }
        ...
        parserTestContainer("Boolean literals") {
            inContext(ExpressionParsingCtx) {
                "true" should parseAs { boolean(true) }

                "false" should parseAs { boolean(false) }
            }
        }

        parserTestContainer("Null literal") {
            inContext(ExpressionParsingCtx) { "null" should parseAs { nullLit() } }
        }
        ...
        parserTestContainer("Numeric literals") {
            inContext(ExpressionParsingCtx) {
                ...
                "1234_5678_9012_3456L" should
                    parseAs { number(LONG) { it::getValueAsLong shouldBe 1234_5678_9012_3456L } }

                "-0X0000_000f" should
                    parseAs { // this is not a float, it's hex
                ...
                "0.5" should parseAs { number(DOUBLE) { it::getBase shouldBe 10 } }

                "0." should parseAs { number(DOUBLE) { it::getBase shouldBe 10 } }
                val doubleOrFloatInBase10: NodeSpec<*> = { number { it::getBase shouldBe 10 } }
        ...
        parserTestContainer("Hex integral literals") {
            inContext(ExpressionParsingCtx) {
                ...
                "-0X0000_000f" should parseAs { unaryExpr(UNARY_MINUS) { hex15i() } }
            }
        }

        parserTestContainer(
            "Binary numeric literals - pre java1.7",
            javaVersions = Earliest..J1_6,
        ) {
