...
fun TypeSystem.mockTypeVar(name: String): JTypeVar {
    val sym =
        mock(JTypeParameterSymbol::class.java).apply {
...
fun mockLexicalScope(vararg names: String): LexicalScope =
    with(testTypeSystem) { LexicalScope.EMPTY.andThen(names.map { mockTypeVar(name = it) }) }
...
private fun LexicalScope.shouldThrowWhenParsingType(
    sig: String,
    matcher: (InvalidTypeSignatureException) -> Unit = {},
) {
    val ex =
        shouldThrow<InvalidTypeSignatureException> {
            val ts = testTypeSystem
...
private fun LexicalScope.shouldParseMethod(
    ...
    withClue("Parsing descriptor") { mt.ensureParsed() }
    ...
    FunSpec({
        ...
        test("Test wildcards") {
            ...
            scope.shouldParseType("Ljava/util/Map<-Ljava/lang/Number;+[Ljava/lang/Object;>;") {
                java.util.Map::class[
                    `?` `super` java.lang.Number::class, `?` extends ts.OBJECT.toArray()]
            }
        }
        ...
        test("Test multiple type bounds") {
            with(LexicalScope.EMPTY) {
                shouldParseMethod(
                    descriptor = "<T:Ljava/lang/CharSequence;:Ljava/lang/Number;>()V"
                ) {
                    it.typeParams.shouldHaveSize(1)
                    val (t) = it.typeParams
                    t::getUpperBound shouldBe
                        ts.glb(java.lang.Number::class.raw, java.lang.CharSequence::class.raw)
                }
            }
        }
        ...
        test("Test inner type") {
            with(LexicalScope.EMPTY) {
                shouldParseType(
                    "Ljavasymbols/testdata/impls/SomeInnerClasses<Ljava/lang/String;>.Inner<Ljava/lang/String;>;"
                ) {
                    SomeInnerClasses::class[ts.STRING].selectInner(
                        SomeInnerClasses.Inner::class.raw.symbol,
                        listOf(ts.STRING),
                    )
                }
            }
        ...
        test("Test throws clause") {
            with(LexicalScope.EMPTY) {
                shouldParseMethod(
                    descriptor = "<T:Ljava/lang/Exception;>()V^TT;^Ljava/lang/Error;"
                ) {
                    it.typeParams.shouldHaveSize(1)
                    val (t) = it.typeParams
                    it::getExceptionTypes shouldBe listOf(t, java.lang.Error::class.raw)
        ...
        test("Test type params builder") {
            ...
            with(MockTypeParamsScanner(aMethodSig)) {
                ...
                withClue("End offset") { e shouldBe aMethodSig.length }

                this.tparams shouldBe listOf("T" to ":Ljava/lang/Object;")
            }
        }
        ...
        test("Test more complicated bounds on type param parser") {
            ...
            with(MockTypeParamsScanner(aMethodSig)) {
                ...
                this.tparams shouldBe listOf("T" to ":Ljava/util/List<-TK;*>;", "K" to ":")
            }
        }
