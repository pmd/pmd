...
class TokenUtilsTest :
    ...
    FunSpec({
        fun TestScope.setup1(assertions: Assertions<List<JavaccToken>>) {
            ...
            fileTokens.map { it.image } shouldBe
                listOf(
        ...
        test("Test nth following token, too far right") {
            setup1 { fileTokens ->
                shouldThrow<NoSuchElementException> { TokenUtils.nthFollower(fileTokens[3], 15) }
            }
        }

        test("Test nth following token, stupid input") {
            setup1 { fileTokens ->
                shouldThrow<IllegalArgumentException> { TokenUtils.nthFollower(fileTokens[3], -15) }
            }
        }
    })
