    ...
    ParserTestSpec({
                                    ...
                                    fieldAccess("f") {
                                        ...
                                        it::getQualifier shouldBe
                                            variableAccess("a") {
                                                ...
                                                it.tokenList().map { token -> token.image } shouldBe
                                                    listOf("(", "(", "a", ")", ")")
                                            }
                                    }
                            }
        ...
        parserTestContainer("Test expressions with complicated LHS") {
            ...
            inContext(ExpressionParsingCtx) {
                ...
                "((int[]) obj)[(int) i]" should
                    parseAs {
                        arrayAccess {
                            it::getQualifier shouldBe
                                parenthesized {
                                    castExpr {
                                        arrayType({ primitiveType(INT) }) { arrayDim() }
                                        variableAccess("obj")
                                    }
                                }

                            it::getIndexExpression shouldBe
                                castExpr {
                                    primitiveType(INT)
                                    variableAccess("i")
                                }
                ...
                "(switch (obj) { case a -> 1; default -> 2; }).length()" should
                    parseAs {
                        methodCall("length") {
                            it::getQualifier shouldBe parenthesized { switchExpr() }
