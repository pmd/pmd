...
class LombokTest :
    IntelliMarker,
    ProcessorTestSpec({
        fun JavaParsingHelper.disableLombok() = withLanguageProperties {
            (this as JavaLanguageProperties).setProperty(
                JavaLanguageProperties.FIRST_CLASS_LOMBOK,
                false,
            )
        }
        ...
        parserTest(
            "Lombok val is inferred in all versions",
            ...
            acu.withTypeDsl {
                acu.varId("q").let {
                    ...
                    it.ancestors(ASTLocalVariableDeclaration::class.java)
                        .firstOrThrow()
                        .isFinal shouldBe true
                }
        ...
        parserTest(
            "Lombok var is inferred (when version <= 9)",
