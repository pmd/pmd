...
/**
 * Tests "the greatest lower bound" (glb). See
 * net.sourceforge.pmd.lang.java.types.TypeSystem.glb(Collection<? extends JTypeMirror>).
 *
        ...
        with(TypeDslOf(testTypeSystem)) { // import construction DSL
            with(gen) { // import constants
                test("Test intersection minimization") {
                    checkAll(ts.subtypesArb()) { (t, s) -> glb(t, s) shouldBe t }
                ...
                test("Test GLB of arrays") {
                    glb(ts.SERIALIZABLE.toArray(), t_ArrayList.toArray()) shouldBe
                        t_ArrayList.toArray()
                    glb(t_ArrayList.toArray(), ts.SERIALIZABLE.toArray()) shouldBe
                        t_ArrayList.toArray()
                ...
                test("Test GLB of arrays of unrelated type") {
                    ...
                    shouldThrow<IllegalArgumentException> {
                        glb(
                            LubTestData.C1::class.decl.toArray(),
                            LubTestData.C2::class.decl.toArray(),
                        )
                    }
                    ...
                    glb(LubTestData.C1::class.decl.toArray(), LubTestData.I1::class.decl.toArray())
                        .shouldBe(
                            (LubTestData.C1::class.decl * LubTestData.I1::class.decl).toArray()
                        )
                }

                test("Test lub of zero types") {
                    shouldThrow<IllegalArgumentException> { ts.glb(emptyList()) }
                }

                test("Test GLB errors") {
                    shouldThrow<IllegalArgumentException> { glb(int, t_Number) }
                    shouldThrow<IllegalArgumentException> { glb(int, ts.OBJECT) }
                }

                test("test corner case") {
                    TypeOps.mostSpecific(
                        setOf(t_Collection[`?` extends t_Number], `t_List{?}`, t_List)
                    ) shouldBe setOf(t_Collection[`?` extends t_Number], `t_List{?}`)
                }

                test("Test GLB corner cases") {

                    // note: intersections are not minimized, or reduced to the null type if they
                    // are unsatisfiable.
                    // note also that in this test we test the components explicitly instead of
                    // using the DSL,
                    // because the DSL operator to create intersections actually calls GLB.

                    glb(t_Iterable[`?` extends t_Number], t_Iterable[t_String]).shouldBeA<
                        JIntersectionType
                    > {
                        it.components.shouldContainExactly(
                            t_Iterable[`?` extends t_Number],
                            t_Iterable[t_String],
                        )
                    }
                    ...
                    glb(
                        ...
                        .shouldBeA<JIntersectionType> {
                            it.components.shouldContainExactlyInAnyOrder(
                                `t_ArrayList{Integer}`,
                                t_Iterable[t_String],
                            )
                        }

                    glb(`t_List{? extends Number}`, `t_Collection{Integer}`).shouldBeA<
                        JIntersectionType
                    > {
                        it.components.shouldContainExactlyInAnyOrder(
                    ...
                    glb(
                        ...
                        `t_Collection{Integer}`,
                        `t_ArrayList{Integer}`,
                    ) shouldBe `t_ArrayList{Integer}`
                    glb(`t_List{? extends Number}`, `t_List{String}`, `t_Enum{JPrimitiveType}`)
                        .shouldBeA<JIntersectionType> {
                            it.components.shouldContainExactlyInAnyOrder(
                    ...
                    glb(t_Collection[`?` extends t_Number], `t_List{?}`, t_List).shouldBeA<
                        JIntersectionType
                    > {
                        it.components.shouldContainExactlyInAnyOrder(
                    ...
                    glb(t_List, `t_List{?}`) shouldBe `t_List{?}`
                    glb(t_Collection[`?` extends t_Number], `t_List{?}`).shouldBeA<
                        JIntersectionType
                    > {
                        it.components.shouldContainExactlyInAnyOrder(
                            t_Collection[`?` extends t_Number],
                ...
                test("Test GLB of recursive types #5096") {
                    val (acu, _) =
                        javaParser.parseWithTypeInferenceSpy(
                            """
