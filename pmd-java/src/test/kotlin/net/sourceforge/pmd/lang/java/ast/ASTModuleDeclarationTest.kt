 ...
 * @since 7.0.0
 */
class ASTModuleDeclarationTest :
    ParserTestSpec({
        parserTest("Test annotations on module", javaVersions = since(J9)) {
            val root: ASTCompilationUnit =
                parser.withProcessing(true).parse("@A @a.B module foo { } ")
        ...
        parserTestContainer("opens decl") {
            inContext(ModuleDirectiveParsingContext) {
                "opens a.package_ to a.module_;" should
                    parseAs {
                ...
                "opens a to a.module_, another;" should
                    parseAs {
                        child<ASTModuleOpensDirective> {
                            it::getPackageName shouldBe "a"
                            it.targetModules.toList() shouldBe
                                listOf(modName("a.module_"), modName("another"))
                        }
                    }
                "opens a;" should
                    parseAs {
        ...
        parserTestContainer("exports decl") {
            inContext(ModuleDirectiveParsingContext) {
                "exports a.package_ to a.module_;" should
                    parseAs {
                ...
                "exports a to a.module_, another;" should
                    parseAs {
                        child<ASTModuleExportsDirective> {
                            it::getPackageName shouldBe "a"
                            it.targetModules.toList() shouldBe
        ...
        parserTestContainer("Test uses") {
            inContext(ModuleDirectiveParsingContext) {
                "uses a.clazz.Name;" should
                    parseAs {
                        child<ASTModuleUsesDirective> { it::getService shouldBe classType("Name") }
                    }
            }
        }
        ...
        parserTestContainer("Test provides") {
            inContext(ModuleDirectiveParsingContext) {
                enableProcessing()
                "provides an.Itf with imp1.Impl;" should
                    parseAs {
                        child<ASTModuleProvidesDirective> {
                            it::getService shouldBe classType("Itf")
                            it.serviceProviders.toList() shouldBe
                                listOf(
                                    classType("Impl") {
...
fun TreeNodeWrapper<Node, *>.modName(
    name: String,
    assertions: NodeSpec<ASTModuleName> = EmptyAssertions,
) =
    child<ASTModuleName> {
