...
class VarScopingTest :
    ProcessorTestSpec({
        ...
        """
            ...
            val (outerClass, innerClass) = acu.descendants(ASTClassDeclaration::class.java).toList()

            val (outerField, localInInit, foreachParam, methodParam, localInBlock, innerField) =
                acu.descendants(ASTVariableId::class.java).crossFindBoundaries().toList()

            val (inInitializer, inForeachInit, inForeach, inMethod, inLocalBlock, inInnerClass) =
                acu.descendants(ASTMethodCall::class.java).crossFindBoundaries().toList()
        ...
        """
            ...
            doTest("Inside try body: r is the resource") { inTry shouldResolveToLocal reader1 }
            ...
            doTest(
                "Inside resource declaration 2: r is the field, before the other resource is declared"
            ) {
        ...
        """
            ...
            val (ivar, _) = acu.descendants(ASTVariableId::class.java).toList()

            val iUsages = acu.descendants(ASTVariableAccess::class.java).toList()
            val (initAccess, condAccess, updateAccess, bodyAccess, innerLoopAccess) = iUsages

            doTest("Usages") { ivar.localUsages shouldBe iUsages }
            doTest("Inside init") { initAccess shouldResolveToLocal ivar }
            doTest("Inside condition") { condAccess shouldResolveToLocal ivar }

            doTest("Inside update") { updateAccess shouldResolveToLocal ivar }

            doTest("Inside body") { bodyAccess shouldResolveToLocal ivar }

            doTest("Inside init of nested loop") { innerLoopAccess shouldResolveToLocal ivar }
        }
        ...
        """
            ...
            val (_, compactCtor, normalCtor) =
                acu.descendants(ASTBodyDeclaration::class.java)
                    .filterIs(SymbolDeclaratorNode::class.java)
                    .toList()
        ...
        """
            ...
            val foreachVar = acu.descendants(ASTVariableId::class.java).first { it.name == "s" }!!

            val foreachBody = acu.descendants(ASTForeachStatement::class.java).firstOrThrow().body
    ...
    """
            ...
            val (enumA, enumB) =
                acu.descendants(ASTEnumDeclaration::class.java)
                    .descendants(ASTVariableId::class.java)
                    .toList()

            val (e, caseA, caseB) = acu.descendants(ASTVariableAccess::class.java).toList()

            val (qualifiedA) = acu.descendants(ASTFieldAccess::class.java).toList()
            ...
            fooCall.qualifier!!.shouldMatchN { ambiguousName("A") }
        }
        ...
        """
            ...
            for (call in acu.methodCalls()) {
                withClue(call) { call.symbolTable.variables().resolve("_").shouldBeEmpty() }
            }
        }
    })
