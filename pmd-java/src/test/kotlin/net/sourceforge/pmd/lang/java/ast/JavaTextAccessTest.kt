...
// equatable to string
private val TextAvailableNode.textStr: String
    get() = text.toString()

class JavaTextAccessTest :
    ParserTestSpec({
                ...
                "int a = ((3));" should
                    parseAs {
                        localVarDecl {
                            ...
                            modifiers { it.textStr shouldBe "" }

                            primitiveType(INT) { it.textStr shouldBe "int" }
                            variableDeclarator("a") {
                                it.textStr shouldBe "a = ((3))"

                                it::getInitializer shouldBe int(3) { it.textStr shouldBe "((3))" }
                            }
                        }
                    }
                    ...
                    parseAs {
                        localVarDecl {
                            ...
                            variableDeclarator("a") {
                                ...
                                it::getInitializer shouldBe
                                    fieldAccess("f") {
                                        it.textStr shouldBe "((a)).f"

                                        it::getQualifier shouldBe
                                            variableAccess("a") { it.textStr shouldBe "((a))" }
                                    }
                            }
                        }
                    }
                                ...
                                it::getInitializer shouldBe
                                    infixExpr(BinaryOp.ADD) {
                                        ...
                                        infixExpr(BinaryOp.ADD) {
                                            ...
                                            int(1) { it.textStr shouldBe "1" }
                                            int(2) { it.textStr shouldBe "2" }
                                        }

                                        variableAccess("f") { it.textStr shouldBe "f" }
                                    }
                            }
                        }
                    }
