...
/**
 * Describes a kind of node that can be found commonly in the same contexts. This type defines some
 * machinery to parse a string to this kind of node without much ado by placing it in a specific
 * parsing context.
 */
abstract class NodeParsingCtx<T : Node>(val constructName: String) {
    ...
    /**
     * Parse the string in the context described by this object. The parsed node is usually the
     * child of the returned [T] node. Note that [parseAndFind] can save you some keystrokes because
     * it finds a descendant of the wanted type.
     *
     * @param construct The construct to parse
     * @return A [T] whose child is the given statement
     * @throws ParseException If the argument is no valid construct of this kind (mind the language
     *   version)
     */
    fun parseNode(
        construct: String,
        ctx: ParserTestCtx,
        parser: JavaParsingHelper = ctx.parser,
    ): T {
    ...
    /**
     * Parse the string the context described by this object, and finds the first descendant of type
     * [N]. The descendant is searched for by [findFirstNodeOnStraightLine], to prevent accidental
     * mis-selection of a node. In such a case, a [NoSuchElementException] is thrown, and you should
     * fix your test case.
     *
     * @param construct The construct to parse
     * @param N The type of node to find
     * @return The first descendant of type [N] found in the parsed expression
     * @throws NoSuchElementException If no node of type [N] is found by
     *   [findFirstNodeOnStraightLine]
    ...
    inline fun <reified N : Node> parseAndFind(construct: String, ctx: ParserTestCtx): N =
        parseNode(construct, ctx).findFirstNodeOnStraightLine(N::class.java)
            ?: throw NoSuchElementException(
                "No node of type ${N::class.java.simpleName} in the given $constructName:\n\t$construct"
            )
...
/**
 * Finds the first descendant of type [N] of [this] node which is accessible in a straight line. The
 * descendant must be accessible from the [this] on a path where each node has a single child.
 *
 * If one node has another child, the search is aborted and the method returns null.
 */
...
object ExpressionParsingCtx : NodeParsingCtx<ASTExpression>("expression") {
    ...
    override fun retrieveNode(acu: ASTCompilationUnit): ASTExpression =
        StatementParsingCtx.retrieveNode(acu).descendants(ASTExpression::class.java).firstOrThrow()
}

object RootParsingCtx : NodeParsingCtx<ASTCompilationUnit>("compilation unit") {

    override fun getTemplate(construct: String, ctx: ParserTestCtx): String = construct
...
object TopLevelTypeDeclarationParsingCtx :
    NodeParsingCtx<ASTTypeDeclaration>("top-level declaration") {
    ...
    override fun retrieveNode(acu: ASTCompilationUnit): ASTTypeDeclaration =
        acu.typeDeclarations.firstOrThrow()
}
...
object AnnotationParsingCtx : NodeParsingCtx<ASTAnnotation>("annotation") {
    ...
    override fun retrieveNode(acu: ASTCompilationUnit): ASTAnnotation =
        StatementParsingCtx.retrieveNode(acu).descendants(ASTAnnotation::class.java).firstOrThrow()
}
