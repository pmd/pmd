...
inline fun <reified T : JVariableSymbol> JSymbolTable.shouldResolveVarTo(
    simpleName: String,
    expected: JVariableSymbol,
): T {
    val resolved =
        variables().resolveFirst(simpleName)
            ?: throw AssertionError("Unresolved variable $simpleName, expected $expected")
    return resolved.symbol.shouldBeA<T> { it shouldBe expected }
}
...
inline fun <reified T : JTypeMirror> JSymbolTable.shouldResolveTypeTo(
    simpleName: String,
    expected: T,
