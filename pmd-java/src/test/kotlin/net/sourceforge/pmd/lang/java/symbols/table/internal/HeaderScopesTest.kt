...
class HeaderScopesTest :
    ProcessorTestSpec({
        ...
        fun JSymbolTable.resolveField(s: String): JFieldSymbol =
            variables().resolveFirst(s)!!.symbol.shouldBeA()
        fun JSymbolTable.resolveMethods(s: String): List<JMethodSymbol> =
            methods().resolve(s).map { it.symbol as JMethodSymbol }

        fun ShadowChain<JTypeMirror, ScopeInfo>.shouldResolveToClass(
            simpleName: String,
            qualName: String,
        ) {
        ...
        fun ShadowChain<JTypeMirror, ScopeInfo>.typeShadowSequence(
            simpleName: String
        ): List<Pair<ScopeInfo, String>> {
            return sequence {
        ...
        parserTest("Test same-package scope") {
            ...
            acu.symbolTable
                .types()
                .shouldResolveToClass("SomeClassA", "javasymbols.testdata.SomeClassA")
        }
        ...
        parserTest("$javalangTypes should be shadowed by $typesInTheSamePackage") {
            ...
            acu.symbolTable.types().typeShadowSequence("Thread") shouldBe
                // from same package
                listOf(
                    SAME_PACKAGE to "javasymbols.testdata.Thread",
        ...
        parserTest("$typesInTheSamePackage should be shadowed by $singleTypeImports") {
            ...
            acu.symbolTable.types().typeShadowSequence("SomeClassA") shouldBe
                // from same package
                listOf(
                    SINGLE_IMPORT to "javasymbols.testdata.SomeClassA",
        ...
        parserTest("$javalangTypes should be shadowed by $singleTypeImports") {
            ...
            acu.symbolTable.types().typeShadowSequence("Thread") shouldBe
                listOf(
                    SINGLE_IMPORT to "javasymbols.testdata.Thread",
                    JAVA_LANG to "java.lang.Thread",
                )
        }
        ...
        parserTest("$onDemandTypeImports should be shadowed by everything") {
            ...
            group.typeShadowSequence("TestCase1") shouldBe
                ...
                listOf(
                    SINGLE_IMPORT to "javasymbols.testdata.TestCase1",
        ...
        parserTest(
            "Types imported through $onDemandStaticImports should be shadowed by $typesInTheSamePackage"
        ) {
            val acu = parser.parseClass(javasymbols.testdata.deep.StaticImportOnDemand::class.java)
            // import javasymbols.testdata.Statics.*;
            ...
            acu.symbolTable.types().typeShadowSequence("PublicShadowed") shouldBe
                // from same package
                listOf(
                    SAME_PACKAGE to "javasymbols.testdata.deep.PublicShadowed",
                    IMPORT_ON_DEMAND to "javasymbols.testdata.Statics\$PublicShadowed",
                )
        }

        parserTest(
            "Types imported through $onDemandStaticImports should be shadowed by $singleTypeImports"
        ) {
        ...
        parserTest(
            "$staticSingleMemberImports should import types, fields and methods with the same name"
        ) {
            val acu = parser.parseClass(javasymbols.testdata.deep.StaticCollisionImport::class.java)
            // import javasymbols.testdata.Statics.*;

            acu.symbolTable.let {
                it.resolveField("Ola") shouldBe
                    classSym(StaticNameCollision::class.java)!!.getDeclaredField("Ola")!!
                it.resolveMethods("Ola")
                    .shouldContainExactly(
                        classSym(StaticNameCollision::class.java)!!
                            .getDeclaredMethods("Ola")
                            .toList()
                    )
                it.types()
                    .shouldResolveToClass("Ola", "javasymbols.testdata.StaticNameCollision\$Ola")
            }
        }
        ...
        parserTest("$staticSingleMemberImports should import inherited members") {
            ...
            acu.symbolTable.let {
                it.resolveField("oha") shouldBe
                    classSym(StaticsSuper::class.java)!!.getDeclaredField("oha")!!
                it.resolveMethods("oha")
                    .shouldContainExactly(
                        classSym(StaticsSuper::class.java)!!.getDeclaredMethods("oha").toList()
                    )
        ...
        parserTestContainer(
            "Method imported through $onDemandStaticImports should be shadowed by $staticSingleMemberImports"
        ) {
            val acu =
                parser.parse(
                    """
                ...
                acu.symbolTable.methods().iterateResults("publicMethod").let {
                    ...
                    it.apply {
                        ...
                        results.forEach { methodSig ->
                            methodSig.symbol.enclosingClass.canonicalName shouldBe
                                "javasymbols.testdata.StaticNameCollision"
                        }
                    }
        ...
        parserTest("Import of an unconventional name with dollar") {
            ...
            with(acu.typeDsl) {
                ...
                moduleAccess.qualifier.shouldBeA<ASTTypeExpression> {
                    it shouldHaveType
                        ts.rawType(ts.getClassSymbol("javasymbols.testdata.deep.ClassWithDollar\$"))
                }
            }
        }
        ...
        parserTest("[java] Incorrect type resolution with classes having the same name #913") {
            val acu1 =
                parser.parseClass(javasymbols.testdata.deep.ClassInDifferentPackage::class.java)
            acu1.symbolTable
                .types()
                .shouldResolveToClass(
                    "ClassInDifferentPackage",
                    "javasymbols.testdata.deep.ClassInDifferentPackage",
                )
            TypeTestUtil.isExactlyA(
                "javasymbols.testdata.deep.ClassInDifferentPackage",
                acu1.typeDeclarations.first(),
            )
            ...
            acu2.symbolTable
                ...
                .shouldResolveToClass(
                    "ClassInDifferentPackage",
                    "javasymbols.testdata.ClassInDifferentPackage",
                )
            TypeTestUtil.isExactlyA(
                "javasymbols.testdata.ClassInDifferentPackage",
                acu2.typeDeclarations.first(),
            )
        }
    })
