...
class ASTSwitchLikeTest extends BaseParserTest {
    ...
    void exhaustiveSwitchSealedClassesAST() {
        Map<String, ASTSwitchLike> switchStatements = java.parse(
                ...
                .collect(Collectors.toMap(
                        s -> s.ancestors(ASTMethodDeclaration.class)
                                .firstOrThrow()
                                .getName(),
                        Function.identity()));
        ...
        assertAll(
                ...
                // Note: the method "notExhaustiveSealed" doesn't actually compile - it is not exhaustive and doesn't
                // have a default
                // the implementation of #isExhaustive uses a shortcut by assuming, the code it sees compiles and
                // assumes it is exhausive...
    ...
    private static void assertSwitch(
            ASTSwitchLike switchLike, boolean isEnum, boolean isExhaustive, boolean hasDefault) {
