...
/**
 * See
 * {@link net.sourceforge.pmd.lang.java.symbols.internal.TypeAnnotReflectionTest}.
 *
...
public class ClassWithTypeAnnotationsInside {
    ...
    @A
    List<String> annotOnList;
    List<@A String> annotOnListArg;
    @A
    List<@A String> annotOnBothListAndArg;

    @A
    int[] annotOnArrayComponent;
    ...
    @A
    Outer.@B Inner1 inner1WithAnnotOnOuterToo;
    @A
    Outer.@B Inner1.Inner2 inner2WithAnnotOnBothOuter;
    @A
    Outer.@A @B Inner1.@B Inner2 inner2WithAnnotOnAll;
    ...
    @A
    OuterG<T, @A T>.Inner5 annotOnOuterGenericArgAndOuter;
    @A
    OuterG<T, @A T>.@A InnerG<@A T> annotOnOuterGenericArgAndInner;

    OuterG<@A ? extends @B String, ? super @A @B T> severalWildcards;
    @A
    OuterG<@A @B ? extends @B String, @A List<@A @B Object>> complicatedField;
