...
/**
 * An {@link OverloadSet} that is specific to an invocation expression and
 * resolution phase. It selects the most specific methods using the actual
 * values of arguments, as specified in
...
final class PhaseOverloadSet extends OverloadSet<MethodCtDecl> {
    ...
    // so try to infer applicability of m2 for argument types == formal param types
    // of m1
    // yeah but this would ignore the shape of lambdas, we'd only get constraints
    // like f1(m1) <: f1(m2), ignoring that a lambda may be compatible with both
    // types
    ...
    private boolean doInfer(JMethodSig m1, JMethodSig m2) {
        ...
        // even if m1 is generic, the type parameters of m1 are treated as type
    ...
    private boolean isFunctionTypeMoreSpecific(InferenceContext ctx, JTypeMirror si, JMethodSig sfun, JMethodSig tfun,
        ...
        // https://docs.oracle.com/javase/specs/jls/se13/html/jls-18.html#jls-18.5.4
    ...
    private boolean addGenericExprConstraintsRecursive(InferenceContext ctx, ExprMirror ei, JTypeMirror rs,
        ...
        if (ei instanceof LambdaExprMirror) {
            ...
            // If RT is void, true.
            //
            // todo: Otherwise, if RS and RT are functional interface types, and ei has at
            // least one result expression, then for each result expression in ei, this
            // entire second step is repeated to infer constraints under which RS is more
            // specific than RT θ' for the given result expression.
            //
            // Otherwise, if RS is a primitive type and RT is not, and ei has at least one
            // result expression, and each result expression of ei is a standalone
            // expression (§15.2) of a primitive type, true.
            //
            // Otherwise, if RT is a primitive type and RS is not, and ei has at least one
            // result expression, and each result expression of ei is either a standalone
            ...
            // If RT is void, true.
            //
            // Otherwise, if RS is a primitive type and RT is not, and the compile-time
            // declaration for ei has a primitive return type, true.
            //
            // Otherwise if RT is a primitive type and RS is not, and the compile-time
