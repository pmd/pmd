...
public class UnusedAssignmentRule extends AbstractJavaRulechainRule {
    ...
    /*
     * Detects unused assignments. This performs a reaching definition analysis.
     * This makes the assumption that there is no dead code.
     * 
     * Since we have the reaching definitions at each variable usage, we could also
     * use that to detect other kinds of bug, eg conditions that are always true, or
     * dereferences that will always NPE. In the general case though, this is
     * complicated and better left to a DFA library, eg google Z3.
     * 
     * This analysis may be used as-is to detect switch labels that fall-through,
     * which could be useful to improve accuracy of other rules.
     * 
     * TODO labels on arbitrary statements (currently only loops) explicit ctor call
     * (hard to impossible without type res, or at least proper graph algorithms
     * like toposort) -> this is pretty invisible as it causes false negatives, not
     * FPs test ternary expr more precise exception handling: since we have access
     * to the overload for method & ctors, we can know where its thrown exceptions
     * may end up in enclosing catches. extract the reaching definition analysis, to
     * exploit control flow information in rules + symbol table. The following are
     * needed to implement scoping of pattern variables, and are already computed by
     * this analysis: whether a switch may fall through whether a statement always
     * completes abruptly whether a statement never completes abruptly because of
     * break
     * 
     * 
     * DONE conditionals loops switch loop labels try/catch/finally lambdas
     * constructors + initializers anon class test this.field in ctors foreach var
     * should be reassigned from one iter to another test local class/anonymous
     * class shortcut conditionals have their own control-flow parenthesized
     * expressions conditional exprs in loops ignore variables that start with
     * 'ignore' ignore params of native methods ignore params of abstract methods
     * 
     */

    private static final PropertyDescriptor<Boolean> CHECK_PREFIX_INCREMENT = PropertyFactory
            .booleanProperty("checkUnusedPrefixIncrement")
            .desc("Report expressions like ++i that may be replaced with (i + 1)").defaultValue(false).build();

    private static final PropertyDescriptor<Boolean> REPORT_UNUSED_VARS = PropertyFactory
            .booleanProperty("reportUnusedVariables")
    ...
    private void reportFinished(DataflowResult result, RuleContext ruleCtx) {
        ...
        for (AssignmentEntry entry : result.getUnusedAssignments()) {
            ...
            final String reason;
            if (killers.isEmpty()) {
                // var went out of scope before being used (no assignment kills it, yet it's
                // unused)
