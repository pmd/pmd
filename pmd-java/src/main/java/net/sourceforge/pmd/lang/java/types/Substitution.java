...
/**
 * A function from {@link SubstVar}s to types. Applying it to a type replaces
 * the occurrences of some variables with other types. This can be done with
 * {@link TypeOps#subst(JTypeMirror, Function)}.
 */
public final class Substitution extends MapFunction<@NonNull SubstVar, @NonNull JTypeMirror> {
    ...
    /**
     * Returns a composed substitution that first applies this substitution to its
     * input, and then applies the {@code after} substitution to the result.
     *
     * <p>
    ...
    /**
     * Maps the given variable to the given type. This does not apply this
     * substitution to the type mirror.
     */
    ...
    /**
     * Builds a substitution where the mapping from vars to types is defined by the
     * correspondence between the two lists.
     * <p>
     * If there are no vars to be mapped, then no substitution is returned even
     * though some types might have been supplied.
     *
    ...
    /**
     * Returns a substitution that replaces the given type variables with their
     * erasure.
     *
     * @param tparams
