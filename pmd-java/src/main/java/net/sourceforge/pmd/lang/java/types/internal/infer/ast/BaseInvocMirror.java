...
abstract class BaseInvocMirror<T extends InvocationNode> extends BasePolyMirror<T> implements InvocationMirror {
    ...
    /**
     * Some method invocations may appear to be poly expressions, but they have no context type (for instance because
     * they are in the initializer of a local with inferred type). These must be treated as standalone expressions.
     */
    protected final boolean mayBePoly;

    BaseInvocMirror(JavaExprMirrors mirrors, T call, boolean mustBeStandalone, @Nullable ExprMirror parent,
    ...
    public List<JTypeMirror> getExplicitTypeArguments() {
        return ASTList.orEmptyStream(myNode.getExplicitTypeArguments()).toStream().map(TypeNode::getTypeMirror)
