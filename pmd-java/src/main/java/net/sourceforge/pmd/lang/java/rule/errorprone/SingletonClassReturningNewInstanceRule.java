...
public class SingletonClassReturningNewInstanceRule extends AbstractJavaRulechainRule {
    ...
    private boolean isDoubleAssignment(ASTVariableAccess variableAccess) {
        ...
        return variableAccess.ancestors(ASTMethodDeclaration.class).descendants(ASTVariableAccess.class)
                ...
                // assignment from a constructor call (next sibling = right hand side) or
                // another Assignment
                .filter(v -> v.getNextSibling() instanceof ASTConstructorCall
                        || v.getNextSibling() instanceof ASTAssignmentExpression)
                ...
                .filter(v -> {
                    ...
                    if (v.getNextSibling() instanceof ASTConstructorCall) {
                        if (v.getParent() instanceof ASTAssignmentExpression
                                && v.getParent().getParent() instanceof ASTAssignmentExpression) {
                            ...
                            if (leftAssignment.getLeftOperand() instanceof ASTAssignableExpr.ASTNamedReferenceExpr) {
                                JVariableSymbol symbol = ((ASTAssignableExpr.ASTNamedReferenceExpr) leftAssignment
                                        .getLeftOperand()).getReferencedSym();
                        ...
                        if (v.getParent() instanceof ASTAssignmentExpression) {
                            ...
                            if (rightAssignment.getLeftOperand() instanceof ASTAssignableExpr.ASTNamedReferenceExpr) {
                                JVariableSymbol symbol = ((ASTAssignableExpr.ASTNamedReferenceExpr) rightAssignment
                                        .getLeftOperand()).getReferencedSym();
