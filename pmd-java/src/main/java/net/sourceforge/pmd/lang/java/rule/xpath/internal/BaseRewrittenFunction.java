...
/**
 * A context node test function that may parse its string argument early if it
 * is a string literal.
 *
 * @param <S>
 *            Type of state into which the argument is parsed
 * @param <N>
 *            Type of node the function applies. The function will return false
...
abstract class BaseRewrittenFunction<S, N extends Node> extends BaseJavaXPathFunction {
    ...
    /**
     * Parse the argument into the state. This is called at build time if the arg is
     * constant, otherwise it's anyway called before
     * {@link #matches(Node, String, Object, boolean)} is called.
     */
