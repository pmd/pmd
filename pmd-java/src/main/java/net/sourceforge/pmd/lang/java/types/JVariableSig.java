...
/**
 * Represents a {@link JVariableSymbol value symbol} viewed in the context of a particular program point (ie under a
 * particular {@link Substitution}).
 *
 * <p>
 * The type given to a symbol is context-dependent. For example, when looking in the supertypes of the current class for
 * a field, we have:
 * 
 * <pre>{@code
 *
 ...
 *
 * <p>
 * This interface plays a similar role to {@link JMethodSig}. It is the type of search results of a
 * {@link JSymbolTable}, see {@link JSymbolTable#variables()}.
 */
public class JVariableSig {
    ...
    /**
     * This is the substituted type. Eg in the example of the class javadoc, for {@code super.field}, this would be
     * {@code Sup<Integer>}. For local variables, this is always the generic type declaration of the enclosing type.
     */
    ...
    public JTypeMirror getTypeMirror() {
        Substitution subst =
                declarator instanceof JClassType ? ((JClassType) declarator).getTypeParamSubst() : Substitution.EMPTY; // array

        JTypeMirror symType =
                declarator.isRaw() ? ClassTypeImpl.eraseToRaw(sym.getTypeMirror(Substitution.EMPTY), subst)
