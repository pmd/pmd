...
public final class JavaRuleUtil {
    ...
    /**
     * Return true if the given expression is enclosed in a zero check. The expression must evaluate to a natural number
     * (ie >= 0), so that {@code e < 1} actually means {@code e == 0}.
     *
     * @param e
     *            Expression
     */
    ...
    /**
     * Returns true if the expression is a stringbuilder (or stringbuffer) append call, or a constructor call for one of
     * these classes.
     *
     * <p>
     * If it is a constructor call, returns false if this is a call to the constructor with a capacity parameter.
     */
    ...
    /**
     * Returns true if the node is a utility class, according to this custom definition.
     */
    ...
    /**
     * Returns true if the string has the given word as a strict prefix. There needs to be a camelcase word boundary
     * after the prefix.
     *
     * <code>
    ...
    /**
     * Returns true if the string has the given word as a word, not at the start. There needs to be a camelcase word
    ...
    /**
     * Whether some variable declared by the given node is a serialPersistentFields (serialization-specific field).
     */
    ...
    /**
     * Whether some variable declared by the given node is a serialVersionUID (serialization-specific field).
     */
    ...
    /**
     * Whether the node or one of its descendants is an expression with side effects. Conservatively, any method call is
     * a potential side-effect, as well as assignments to fields or array elements. We could relax this assumption with
     * (much) more data-flow logic, including a memory model.
     *
     * <p>
     * By default assignments to locals are not counted as side-effects, unless the lhs is in the given set of symbols.
     *
     * @param node
     *            A node
    ...
    /**
     * Returns true if the expression has side effects we don't track. Does not recurse into sub-expressions.
     */
