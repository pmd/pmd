...
/**
 * Resolves symbols from their global name. This abstracts over whether we're looking on a classpath, in a file tree, in
 * a serialized index, etc.
 */
public interface SymbolResolver {

    /**
     * Resolves a class symbol from its canonical name. Periods ('.') will not be interpreted as nested-class
     * separators, so this performs at most one classloader lookup. Note that external symbol resolvers do not need to
     * implement lookup for primitive types, for local and anonymous classes, or for array classes. This is handled by
     * the AST implementation or by the type system. Looking up such symbols is undefined behaviour.
     */
    @Nullable
    ...
    /**
     * Resolves a class symbol from its canonical name. Periods ('.') may be interpreted as nested-class separators, so
     * for n segments, this performs at most n classloader lookups.
     */
    @Nullable
