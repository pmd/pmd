...
public class UnnecessaryFullyQualifiedNameRule extends AbstractJavaRulechainRule {
    ...
    private static final PropertyDescriptor<Boolean> REPORT_METHODS = booleanProperty("reportStaticMethods").desc(
            "Report unnecessary static method qualifiers like in `Collections.emptyList()`, if the method is imported or inherited.")
            .defaultValue(true).build();

    private static final PropertyDescriptor<Boolean> REPORT_FIELDS = booleanProperty("reportStaticFields").desc(
    ...
    /**
     * Checks that the type name can be referred to by simple name in the given
     * scope, which means, that the qualification can be dropped. If the symbol
     * table for types yields the same symbol when referred to by simple name, then
     * this is true.
     *
    ...
    private static @Nullable ScopeInfo typeMeansSame(@NonNull ASTClassType typeNode) {
        ...
        if (symTable.variables().resolveFirst(sym.getSimpleName()) != null) {
            return null; // name is obscured:
                         // https://docs.oracle.com/javase/specs/jls/se15/html/jls-6.html#jls-6.4.2
        }

        return fieldOrTypeMeansSame(sym, typeNode.getSymbolTable(), JSymbolTable::types,
    ...
    private static ScopeInfo fieldMeansSame(ASTFieldAccess field) {
        ...
        return fieldOrTypeMeansSame(sym, field.getSymbolTable(), JSymbolTable::variables,
                (s, t) -> s.equals(t.getSymbol()));
    }

    private static <S extends JElementSymbol, T> ScopeInfo fieldOrTypeMeansSame(@NonNull S originalSym,
            JSymbolTable symTable, Function<JSymbolTable, ShadowChain<T, ScopeInfo>> shadowChainGetter,
            BiPredicate<S, T> areEqual) {

        ShadowChainIterator<T, ScopeInfo> iter = shadowChainGetter.apply(symTable)
                .iterateResults(originalSym.getSimpleName());
    ...
    /**
     * Return true if removing the qualification from this field access would
     * produce an "Illegal forward reference" compiler error. This would happen if
     * the referenced field is defined after the reference, in the same class. Note
     * that the java compiler uses definite assignment to find forward references.
     * Here we over-approximate this, to avoid depending on the dataflow pass. We
     * could fix this later though.
     *
     * @param fieldAccess
     *            A field access
     */
    private static boolean isForwardReference(ASTFieldAccess fieldAccess) {
        ...
        ASTBodyDeclaration enclosing = fieldAccess.ancestors(ASTBodyDeclaration.class).first();
        if (isPartOfStaticInitialization(enclosing)
            ...
            int declIndex = fieldDecl.ancestors().filter(it -> it.getParent() instanceof ASTTypeBody).firstOrThrow()
                    .getIndexInParent();
