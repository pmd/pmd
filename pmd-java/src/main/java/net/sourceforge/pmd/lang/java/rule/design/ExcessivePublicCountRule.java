 ...
 *
 * <p>
 * If a class has a high number of public operations, it might be wise to consider whether it would be appropriate to
 * divide it into subclasses.
 * </p>
 *
 * <p>
 * A large proportion of public members and operations means the class has high potential to be affected by external
 * classes. Futhermore, increased effort will be required to thoroughly test the class.
 * </p>
 *
 * @author aglover
 */
public class ExcessivePublicCountRule extends AbstractJavaCounterCheckRule<ASTTypeDeclaration> {
    ...
    protected boolean isViolation(ASTTypeDeclaration node, int reportLevel) {
        long publicCount = node.getDeclarations().filterIs(ModifierOwner.class).filter(it -> it.hasModifiers(PUBLIC))
                // filter out constants
                .filter(it -> !(it instanceof ASTFieldDeclaration && it.hasModifiers(STATIC, FINAL))).count();
