 ...
 * and not a structural distinction in the AST.
 *
 * <p>
 * This node can be converted to a list with {@link #toList()}. Often these nodes are optional in their parent, and so
 * might be null. The method {@link ASTList#orEmpty(ASTList) orEmpty} helps in such cases. For example
 * 
 * <pre>{@code
 ...
 *
 * <p>
 * Note that though it is usually the case that the node lists all its children, there is no guarantee about that. For
 * instance, {@link ASTFormalParameters} excludes the {@linkplain ASTReceiverParameter receiver parameter}.
 *
 * @param <N>
...
public abstract class ASTList<N extends JavaNode> extends AbstractJavaNode implements Iterable<N> {
    ...
    /**
     * Returns the number of nodes in this list. This must be the number of nodes yielded by the {@link #iterator()}.
     */
    public int size() {
    ...
    /**
     * Returns an empty list if the parameter is null, otherwise returns its {@link #toList()}.
     *
     * @param list
     *            List node
     * @param <N>
    ...
    /**
     * Super type for *nonempty* lists that *only* have nodes of type {@code <T>} as a child.
     */
