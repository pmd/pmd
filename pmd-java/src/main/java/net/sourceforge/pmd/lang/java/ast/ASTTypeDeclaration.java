 ...
 *
 * <p>
 * Note: In PMD 6, there was a node with this name (ASTTypeDeclaration) which was a top-level wrapper node around type
 * declarations. This node has been removed in PMD 7 and the name has been reused.
 */
public interface ASTTypeDeclaration extends TypeNode, ModifierOwner, TypeParamOwnerNode, ASTBodyDeclaration,
        ASTTopLevelDeclaration, JavadocCommentOwner {
    ...
    /**
     * Returns the {@linkplain JClassType#getGenericTypeDeclaration() generic type declaration} of the declared type.
     * Note that for {@linkplain ASTAnonymousClassDeclaration anonymous classes}, this returns a class type whose symbol
     * is the actual anonymous class. Eg {@code new Runnable() { void foo() { } void run() { } }} would present both
     * methods, ie not just be a {@code Runnable}. The {@link ASTConstructorCall} would have type {@code Runnable}
    ...
    /**
     * Returns the simple name of this type declaration. Returns the empty string if this is an anonymous class
     * declaration.
     */
    @NonNull
    ...
    /**
     * Returns the binary name of this type declaration. This is like {@link Class#getName()}.
     */
    @NonNull
    String getBinaryName();

    /**
     * Returns the canonical name of this class, if it exists. Otherwise returns null. This is like
     * {@link Class#getCanonicalName()}.
     *
     * <p>
     * A canonical name exists if all enclosing types have a canonical name, and this is neither a local class nor an
     ...
     * Anonymous classes are not considered local. Only class declarations can be local. Local classes cannot be static.
     */
    ...
    /**
     * Returns true if this is an interface type declaration (including annotation types). This is consistent with
     * {@link Class#isInterface()}.
     */
     ...
     * not an implicit class). Note that eg {@link JClassSymbol#isClass()} counts records and enums in, just like
    ...
    /**
     * Returns true if this is a regular interface declaration (not an annotation). Note that {@link #isInterface()}
     * counts annotations in.
     */
    ...
    /**
     * Returns the list of interfaces implemented by this class, or extended by this interface. Returns null if no such
     * list is declared.
     */
    ...
    /**
     * Returns the superclass type node if this node is a class declaration and explicitly declares an {@code extends}
     * clause. Superinterfaces of an interface are not considered. Note that enum or record declarations never have an
     * explicit superclass type node. Anonymous class declarations have such a type node if the class being created is a
     * class, otherwise, it is an interface, and the superclass is implicitly object, so this method returns null. Enum
     * constants with a subclass body will return null.
     */
    ...
    /**
     * Return the explicit permits list if there is one. Note that the permitted subtypes list may be implicit and
     * inferred from subtypes found in the current compilation unit. Use {@link JClassSymbol#getPermittedSubtypes()} for
