...
public class UseUtilityClassRule extends AbstractJavaRulechainRule {
    ...
    private static final Set<String> IGNORED_CLASS_ANNOT = setOf("lombok.experimental.UtilityClass",
    ...
    public Object visit(ASTClassDeclaration klass, Object data) {
        if (JavaAstUtils.hasAnyAnnotation(klass, IGNORED_CLASS_ANNOT)
                ...
                || klass.isInterface() || klass.isAbstract() || klass.getSuperClassTypeNode() != null
        ...
        hasNonPrivateCtor |= !hasAnyCtor && klass.getVisibility() != Visibility.V_PRIVATE
    ...
    private boolean hasLombokPrivateCtor(ASTClassDeclaration parent) {
        // check if there's a lombok no arg private constructor, if so skip the rest of
        // the rules

        return parent.getDeclaredAnnotations().filter(t -> TypeTestUtil.isA("lombok.NoArgsConstructor", t))
                .flatMap(ASTAnnotation::getMembers)
                // to set the access level of a constructor in lombok, you set the access
                // property on the annotation
                .filterMatching(ASTMemberValuePair::getName, "access")
                // This is from the AccessLevel enum in Lombok
                // if the constructor is found and the accesslevel is private no need to check
                // anything else
                .any(it -> isAccessToVarWithName(it.getValue(), "PRIVATE"));
    }

    private static boolean isAccessToVarWithName(JavaNode node, String name) {
        return node instanceof ASTNamedReferenceExpr && ((ASTNamedReferenceExpr) node).getName().equals(name);
    }
