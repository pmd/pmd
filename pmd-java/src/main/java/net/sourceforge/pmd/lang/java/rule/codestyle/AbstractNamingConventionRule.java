...
/**
 * Base class for naming conventions rule. Not public API, but used to
 * uniformize eg property names between our rules.
 *
 * <p>
 * Protected methods may leak API because concrete classes are not final so
 * they're package private instead
 *
...
abstract class AbstractNamingConventionRule<T extends JavaNode> extends AbstractJavaRulechainRule {
    ...
    /**
     * The argument is interpreted as the display name, and is converted to camel
     * case to get the property name.
     */
    RegexPropertyBuilder defaultProp(String displayName) {
        return defaultProp(CaseConvention.SPACE_SEPARATED.convertTo(CaseConvention.CAMEL_CASE, displayName),
                displayName);
    }

    /**
     * Returns a pre-filled builder with the given name and display name (for the
     * description).
     */
    RegexPropertyBuilder defaultProp(String name, String displayName) {
        return PropertyFactory.regexProperty(name + "Pattern")
                .desc("Regex which applies to " + displayName.trim() + " names").defaultValue(defaultConvention());
    }
    ...
    void checkMatches(T node, PropertyDescriptor<Pattern> regex, Object data) {
        ...
        if (!getProperty(regex).matcher(name).matches()) {
            asCtx(data).addViolation(node, kindDisplayName(node, regex), name, getProperty(regex).toString());
        }
    }
