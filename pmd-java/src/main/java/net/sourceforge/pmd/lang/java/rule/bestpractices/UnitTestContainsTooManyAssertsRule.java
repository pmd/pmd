...
public class UnitTestContainsTooManyAssertsRule extends AbstractJavaRulechainRule {
    ...
    private static final PropertyDescriptor<Integer> MAX_ASSERTS =
            PropertyFactory.intProperty("maximumAsserts").desc("Maximum number of assert calls in a test method")
                    .require(NumericConstraints.positive()).defaultValue(1).build();

    private static final PropertyDescriptor<Set<String>> EXTRA_ASSERT_METHOD_NAMES =
            PropertyFactory.stringProperty("extraAssertMethodNames").desc("Extra valid assertion methods names")
    ...
    public Object visit(ASTMethodDeclaration method, Object data) {
        ...
        if (body != null && TestFrameworksUtil.isTestMethod(method)) {
            ...
            int assertCount =
                    body.descendants(ASTMethodCall.class).filter(call -> TestFrameworksUtil.isProbableAssertCall(call)
                            || extraAsserts.contains(call.getMethodName())).count();
