...
public enum JModifier {
    ...
    /**
     * Returns the constant of java.lang.reflect.Modifier that this modifier corresponds to. Be aware that the following
     * constants are source-level modifiers only, for which this method returns 0:
     * <ul>
     * <li>{@link #DEFAULT}: this doesn't exist at the class file level. A default method is a non-static non-abstract
     * public method declared in an interface ({@link JMethodSymbol#isDefaultMethod()}.
     * <li>{@link #SEALED}: a sealed class has an attribute {@code PermittedSubclasses} with a non-zero length (in the
     * compiled class file). ({@link JClassSymbol#isSealed()})
     * <li>{@link #NON_SEALED}: this doesn't exist at the class file level at all. But a class must have the non-sealed
     * modifier in source if it is neither sealed, nor final, and appears in the {@code PermittedSubclasses} attribute
