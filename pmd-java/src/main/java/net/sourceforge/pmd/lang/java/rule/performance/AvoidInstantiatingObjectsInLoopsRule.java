...
public class AvoidInstantiatingObjectsInLoopsRule extends AbstractJavaRulechainRule {
     ...
     * @return boolean This returns whether the given constructor call is part of a
     *         throw statement
     */
     ...
     * @param node
     *            This is the expression of part of java code to be checked.
     * @return boolean <code>false</code> if the given node is inside a loop,
    ...
    private boolean notInsideLoop(Node node) {
        ...
        while (n != null) {
            if (n instanceof ASTLoopStatement) {
                ...
                /*
                 * init part is not technically inside the loop. Skip parent ASTForStatement but
                 * continue higher up to detect nested loops
                 */
                n = n.getParent();
