...
/**
 * Root context object for type analysis. Type systems own a global {@link SymbolResolver}, which creates and caches
 * external symbols. Methods of this class promote symbols to types, and compose types together. {@link TypeOps} and
 * {@link TypeConversion} have some more operations on types.
 *
 * <p>
 * Some special types are presented as constant fields, eg {@link #OBJECT} or {@link #NULL_TYPE}. These are always
 * comparable by reference. Note that the primitive wrapper types are not exposed as constants here, but can be accessed
 * by using the {@link JTypeMirror#box() box} method on some primitive constant.
 *
 * <p>
 * The lifetime of a type system is the analysis: it is shared by all compilation units. TODO this is hacked together by
 * comparing the ClassLoader, but this should be in the language instance
 *
 * <p>
 * Nodes have a reference to the type system they were created for: {@link JavaNode#getTypeSystem()}.
 */
@SuppressWarnings("PMD.CompareObjectsWithEquals")
public final class TypeSystem {

    /**
     * Top type of the reference type system. This is the type for the {@link Object} class. Note that even interfaces
     * have this type as a supertype ({@link JTypeMirror#getSuperTypeSet()}).
     */
    public final JClassType OBJECT;

    /**
     * The bottom type of the reference type system. This is named the <i>null type</i> in the JLS and is not denotable
     * in Java programs.
     *
     * <p>
    ...
    /**
     * A constant to represent the normal absence of a type. The primitive {@code void.class} represents that type, and
     * this is the return type of a void method.
     *
     * <p>
     * Note that the type of the class literal {@code void.class} is {@code Class<java.lang.Void>}, not NO_TYPE.
     *
     * <p>
     * {@code java.lang.Void} is represented by {@link #BOXED_VOID}. Note that {@code BOXED_VOID.unbox() != NO_TYPE},
     * {@code NO_TYPE.box() != BOXED_VOID}.
     *
     * <p>
     * {@code NO_TYPE.isPrimitive()} returns false, even though {@code void.class.isPrimitive()} returns true.
     */
    public final JTypeMirror NO_TYPE;

    /**
     * A constant to represent an unresolved type. This means, that resolution was attempted but failed and shouldn't be
     * tried again. The symbol is a {@link JClassSymbol}.
     *
     * <p>
     * Note that {@link TypeOps#isConvertible(JTypeMirror, JTypeMirror)} considers this type a subtype of anything, even
     * primitive types.
     */
    public final JTypeMirror UNKNOWN;

    /**
     * A constant to represent a typing error. This would have been reported by a compiler, and is used to propagate
     * errors.
     *
     * <p>
     * Note that {@link TypeOps#isConvertible(JTypeMirror, JTypeMirror)} considers this type a subtype of anything, even
    ...
    /**
     * Sentinel value for an unresolved method. This type corresponds to a method declaration in the type
     * {@link #UNKNOWN}, returning {@link #UNKNOWN}.
     */
    ...
    /**
     * This is the boxed type of {@code Void.class}, not to be confused with {@code void.class}, which in this framework
     * is represented by {@link #NO_TYPE}.
     *
     * <p>
     ...
     *            A function that creates a new symbol resolver, which will be owned by the new type system. Because of
     *            cyclic dependencies, the new type system is leaked before its initialization completes, so fields of
     *            the type system are unusable at that time. The resolver is used to create some shared types:
     *            {@link #OBJECT}, {@link #CLONEABLE}, {@link #SERIALIZABLE}, {@link #BOXED_VOID}.
     */
    ...
    /**
     * Returns the class symbol for the given reflected class. This asks the classloader of this type system. Returns
     * null if the parameter is null, or the class is not available in the analysis classpath.
     *
     * @param clazz
     *            Class
     */
    ...
    /**
     * Returns a symbol for the binary name. Returns null if the name is null or the symbol is not found on the
     * classpath. The class must not be an array.
     *
     * @param binaryName
     *            Binary name
     *
    ...
    /**
     * Returns a symbol for the canonical name. Returns null if the name is null or the symbol is not found on the
     * classpath. The class must not be an array.
     *
     * <p>
     * Canonical names separate nested classes with {@code .} (periods) instead of {@code $} (dollars) as the JVM does.
     * Users usually use canonical names, but lookup is much more costly.
     *
     * @param canonicalName
     *            Canonical name
     *
    ...
    /**
     * Returns a type mirror for the given symbol. If the symbol declares type parameters, then the resulting type is
     * raw (differs from the behaviour of {@link #declaration(JClassSymbol)}), meaning all its supertypes are erased.
     *
     * <p>
     * If the symbol is a {@link JTypeParameterSymbol type parameter}, returns a {@link JTypeVar}.
     *
     * <p>
     * If the symbol is a {@link JClassSymbol}, then:
     * <ul>
     * <li>If it represents a primitive type, the corresponding {@link JPrimitiveType} is returned (one of {@link #INT},
     * {@link #CHAR}, etc.).
     * <li>If it represents an array type, a new {@link JArrayType} is returned. The component type will be built with a
     * recursive call.
     * <li>If it represents a class or interface type, a {@link JClassType} is returned.
     * <ul>
     * <li>If the parameter {@code isErased} is true, and if the symbol declares type parameters, then it will be a
     * {@linkplain JClassType#isRaw() raw type}. This means, which means all its generic supertypes are
     * {@linkplain JClassType#hasErasedSuperTypes() erased}.
     * <li>Otherwise, the generic supertypes are preserved. In particular, if the symbol declares type parameters
     * itself, then it will be a {@linkplain JClassType#isGenericTypeDeclaration() generic type declaration}.
     * </ul>
     * If the symbol is a non-static member of another class, then the given type's
     * {@linkplain JClassType#getEnclosingType() enclosing type} is created, applying the above rules about erasure
     * recursively. A type is either completely erased, or completely parameterized.</li>
     * </ul>
     *
     * @param symbol
     *            Symbol for the type declaration
     * @param isErased
     *            Whether the type should be consider erased, if it represents a class or interface type. This does not
    ...
    /**
     * Like {@link #typeOf(JTypeDeclSymbol, boolean)}, defaulting the erased parameter to true. If the symbol is not
     * generic, the returned symbol is not actually raw.
     *
     * @param klass
     *            Symbol
     *
    ...
    /**
     * Like {@link #typeOf(JTypeDeclSymbol, boolean)}, defaulting the erased parameter to false. If the symbol is not
     * generic, the returned symbol is not actually a generic type declaration.
     *
     * @param klass
     *            Symbol
     *
    ...
    /**
     * Produce a parameterized type with the given symbol and type arguments. The type argument list must match the
     * declared formal type parameters in length. Non-generic symbols are accepted by this method, provided the argument
     * list is empty. If the symbol is unresolved, any type argument list is accepted.
     *
     * <p>
     * This method is equivalent to {@code rawType(klass).withTypeArguments(typeArgs)}, but that code would require a
     * cast.
     *
     * @param klass
     *            A symbol
     ...
     * @throws IllegalArgumentException
     *             If the element type is a {@link JWildcardType}, the null type, or {@link #NO_TYPE void}.
    ...
    /**
     * Maps a type decl symbol to its shared representation. Eg this maps the symbol for {@code int.class} to
     * {@link #INT}. Only non-generic types are cached.
     */
    ...
    /**
     * The least upper bound, or "lub", of a set of reference types is a shared supertype that is more specific than any
     ...
     * <li>No two types in the intersection are subtypes of one another (the intersection is minimal):
     * {@code A <: B => (A & B) = A}, in particular, {@code (A & A) = A}
     * <li>The intersection has a single component that is a class, array, or type variable. If all components are
     * interfaces, then that component is {@link #OBJECT}.
     * <li>If several components are arrays, then their components are intersected: {@code A[] & B[] = (A & B)[]}
     * </ul>
     *
     * <p>
     * If after these transformations, only a single component remains, then that is the returned type. Otherwise a
     * {@link JIntersectionType} is created. Note that the intersection may be unsatisfiable (eg
     * {@code A[] & Runnable}), but we don't attempt to minimize this to {@link #NULL_TYPE}. Similarly, we do not
     * attempt to minimize valid intersections. For instance {@code List<?> & Collection<Number>} can technically be
     * minimized to {@code List<Number>}, but doing this requires inference of a fitting parameterization in general,
     * which is complex, and not necessary in the internal tasks where intersection types are useful. In fact
     * intersection types are precisely useful because they are simple to build.
     *
     * <p>
     * See also JLS§4.9 (Intersection types).
     *
     * @throws IllegalArgumentException
     *             If some component is not a class, interface, array, or type variable
     * @throws IllegalArgumentException
    ...
    /**
     * Returns a new type variable for the given symbol. This is only intended to be used by the implementor of
     * {@link JTypeParameterSymbol}.
     */
