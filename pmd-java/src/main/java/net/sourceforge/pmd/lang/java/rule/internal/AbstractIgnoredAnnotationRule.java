...
public abstract class AbstractIgnoredAnnotationRule extends AbstractJavaRule {
    ...
    private final PropertyDescriptor<List<String>> ignoredAnnotationsDescriptor = stringListProperty(
            "ignoredAnnotations").desc(defaultIgnoredAnnotationsDescription())
    ...
    /**
     * Checks whether any annotation in ignoredAnnotationsDescriptor is present on
     * the node.
     *
     * @param node
     *            the node to check
     * @return <code>true</code> if the annotation has been found, otherwise
     *         <code>false</code>
     */
