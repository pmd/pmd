 ...
 * </pre>
 * 
 * (for the same reason) is quite inefficient as trim() causes a new String to be created. A Smarter code to check for
 ...
 * }</pre>
 * 
 * or you can refer to Apache's <code>StringUtils#isBlank</code> (in commons-lang), Spring's
 * <code>StringUtils#hasText</code> (in the Spring framework) or Google's <code>CharMatcher#whitespace</code> (in Guava)
 * for existing implementations (some might include the check for != null).
 *
 * @author acaplan
 */
public class InefficientEmptyStringCheckRule extends AbstractJavaRulechainRule {
    ...
    private static boolean isLengthZeroCheck(ASTMethodCall call) {
        return "length".equals(call.getMethodName()) && call.getArguments().size() == 0
    ...
    private static boolean isTrimCall(ASTExpression expr) {
        if (expr instanceof ASTMethodCall) {
            ...
            return "trim".equals(call.getMethodName()) && call.getArguments().size() == 0
