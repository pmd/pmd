...
class ConditionalMirrorImpl extends BasePolyMirror<ASTConditionalExpression> implements BranchingMirror {
    ...
    /**
     * Conditional expressions are standalone iff both their branches are of a
     * primitive type (or a primitive wrapper type), or they appear in a cast
     * context. This may involve inferring the compile-time declaration of a method
     * call.
     *
    ...
    private JTypeMirror getConditionalStandaloneType(ConditionalMirrorImpl mirror, ASTConditionalExpression cond) {
        ...
        // Let T1
        // be the type that results from applying boxing conversion to S1, and let T2 be
        // the type
        // that results from applying boxing conversion to S2. The type of the
    ...
    private JTypeMirror standaloneExprTypeInConditional(ExprMirror mirror, ASTExpression e) {
        ...
        if (mirror instanceof CtorInvocationMirror) {
            ...
            // A class instance creation expression (§15.9) for a class that is convertible
            // to a numeric type.
