 ...
 *
 * @see <a href="https://docs.oracle.com/javase/specs/jls/se17/html/jls-6.html#jls-6.3.1">Java Language
 *      Specification</a>
 */
final class PatternBindingsUtil {
    ...
    /**
     * Returns the binding set declared by the expression. Note that only some expressions contribute bindings, and
     * every other form of expression does not contribute anything (meaning, all subexpressions are not processed).
     */
    static BindSet bindersOfExpr(ASTExpression e) {
        /*
         * JLS 17§6.3.1 If an expression is not a conditional-and expression, conditional-or expression, logical
         * complement expression, conditional expression, instanceof expression, switch expression, or parenthesized
         * expression, then no scope rules apply.
         */

        if (e instanceof ASTUnaryExpression) {
            ASTUnaryExpression unary = (ASTUnaryExpression) e;
            return unary.getOperator() == UnaryOp.NEGATION ? bindersOfExpr(unary.getOperand()).negate() : BindSet.EMPTY;
    ...
    static BindSet bindersOfPattern(ASTPattern pattern) {
        if (pattern instanceof ASTTypePattern) {
            ...
            return ((ASTRecordPattern) pattern).getComponentPatterns().toStream().reduce(BindSet.EMPTY,
                    (bs, pat) -> bs.union(bindersOfPattern(pat)));
        }
    ...
    /**
     * A set of bindings contributed by a (boolean) expression. Different bindings are introduced if the expr evaluates
     * to false or true, which is relevant for the scope of bindings introduced in if stmt conditions.
     */
    static final class BindSet {
        ...
        public BindSet union(BindSet bindSet) {
            ...
            return new BindSet(trueBindings.plusAll(bindSet.trueBindings),
