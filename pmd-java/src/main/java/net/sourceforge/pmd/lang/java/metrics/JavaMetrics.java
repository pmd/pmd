...
/**
 * Built-in Java metrics. See {@link Metric} and {@link MetricsUtil} for usage doc.
 *
 * @see "Michele Lanza and Radu Marinescu. <i>Object-Oriented Metrics in Practice: Using Software Metrics to
 *      Characterize, Evaluate, and Improve the Design of Object-Oriented Systems.</i> Springer, Berlin, 1 edition,
 *      October 2006."
 */
public final class JavaMetrics {

    /**
     * Number of usages of foreign attributes, both directly and through accessors. "Foreign" hier means "not belonging
     * to {@code this}", although field accesses to fields declared in the enclosing class are not considered foreign.
     *
     * <p>
     * High values of ATFD (&gt; 3 for an operation) may suggest that the class or operation breaks encapsulation by
     * relying on the internal representation of the classes it uses instead of the services they provide.
     */
    ...
    /**
     * Number of independent paths through a block of code. Formally, given that the control flow graph of the block has
     * n vertices, e edges and p connected components, the cyclomatic complexity of the block is given by
     * {@code CYCLO = e - n + 2p}. In practice it can be calculated by counting control flow statements following the
     ...
     * <li>+1 for every control flow statement (if, case, catch, throw, do, while, for, break, continue) and conditional
     * expression (?:). Notice switch cases count as one, but not the switch itself: the point is that a switch should
     * have the same complexity value as the equivalent series of if statements.
     * <li>else, finally and default do not count;
     * <li>+1 for every boolean operator ({@code &&}, {@code ||}) in the guard condition of a control flow statement.
     * That’s because Java has short-circuit evaluation semantics for boolean operators, which makes every boolean
    ...
    /**
     * Cognitive complexity is a measure of how difficult it is for humans to read and understand a method. Code that
     * contains a break in the control flow is more complex, whereas the use of language shorthands doesn't increase the
     * level of complexity. Nested control flows can make a method more difficult to understand, with each additional
     * nesting of the control flow leading to an increase in cognitive complexity.
     *
     * <p>
     ...
     * </p>
     * The following structures receive a nesting increment commensurate with their nested depth inside nested
     * structures:
     * <ul>
    ...
    /**
     * This counts the number of other classes a given class or operation relies on. Classes from the package
     * {@code java.lang} are ignored by default (can be changed via options). Also primitives are not included into the
     * count.
     *
     * <p>
     * Code example:
    ...
    /**
     * Simply counts the number of lines of code the operation or class takes up in the source. This metric doesn’t
     * discount comments or blank lines. See {@link #NCSS} for a less biased metric.
     */
     ...
     * opening braces in the program. Comments and blank lines are ignored, and statements spread on multiple lines
     * count as only one (e.g. {@code int\n a;} counts a single statement).
     *
     * <p>
     * The standard version of the metric is based off [JavaNCSS](http://www.kclee.de/clemens/java/javancss/):
     * <ul>
     * <li>+1 for any of the following statements: {@code if}, {@code else}, {@code while}, {@code do}, {@code for},
     * {@code switch}, {@code break}, {@code continue}, {@code return}, {@code throw}, {@code synchronized},
     * {@code catch}, {@code finally}.
     * <li>+1 for each assignment, variable declaration (except for loop initializers) or statement expression. We count
     * variables declared on the same line (e.g. {@code int a, b, c;}) as a single statement.
     * <li>Contrary to Sonarqube, but as JavaNCSS, we count type declarations (class, interface, enum, annotation), and
     * method and field declarations.
     * <li>Contrary to JavaNCSS, but as Sonarqube, we do not count package declaration and import declarations as
     * statements. This makes it easier to compare nested classes to outer classes. Besides, it makes for class metric
     * results that actually represent the size of the class and not of the file. If you don’t like that behaviour, use
     * the {@link NcssOption#COUNT_IMPORTS} option.
     * </ul>
     *
     * <pre>{@code
     ...
     * metrics don’t count the same thing: NPath counts the number of distinct full paths from the beginning to the end
     * of the method, while Cyclo only counts the number of decision points. NPath is not computed as simply as
     * {@link #CYCLO}. With NPath, two decision points appearing sequentially have their complexity multiplied.
     *
     * <p>
     * The fact that NPath multiplies the complexity of statements makes it grow exponentially: 10 {@code if .. else}
     * statements in a row would give an NPath of 1024, while Cyclo would evaluate to 20. Methods with an NPath
     * complexity over 200 are generally considered too complex.
     *
     * <p>
     * NPath is computed through control flow analysis. We walk a block and keep track of how many control flow paths
     * lead to the current program point. We make sure to separate paths that end abruptly (for instance because of a
     * throw, or return). This accurately counts the number of paths that lead out of a given method. For instance:
     * 
     * <pre>{@code
     * // entry
     * if (foo)
     ...
     * }</pre>
     * 
     * Here there are two paths from the entry to the exit of the method: one that ends after doSomething(), and another
     * that ends with the return statement. Complexity can snowball rapidly. For instance:
     * 
     * <pre>{@code
     * // entry
     * if (foo)
     ...
     * }</pre>
     * 
     * Here there are two paths from {@code entry} to {@code join}: one that goes through each branch of the if/else.
     * Then there are two paths from {@code join} to {@code exit}, for the same reason. The total number of paths from
     * {@code entry} to {@code exit} is {@code 2 * 2 = 4}. Since complexities multiply in this way, it can grow
     * exponentially. This is not the case with early-return patterns for instance:
     * 
     * <pre>{@code
     *  // entry
     *  if (foo) return x;
     ...
     * }</pre>
     * 
     * Here there is only one path from {@code entry} to {@code join}, because the {@code if} branch returns early.
     * There are two paths from {@code join} to {@code exit} (and notice, that's true even if there is no else branch,
     * because the path where the if condition is falso must be taken into account anyway). So in total there are
     * {@code 1*2 + 1 = 3} paths from {@code entry} to the end of the block or function (the return statement still
     * counts).
     *
     * <p>
     * Note that shortcut boolean operators are counted as control flow branches, especially if they happen in the
     * condition of a control flow statement. For instance
     * 
     * <pre>{@code
     * // entry
     * if (foo || bar)
     ...
     * }</pre>
     * 
     * How many paths are there here? There is one path that goes from {@code entry} to {@code exit}, which is taken if
     * {@code !foo && !bar}. There is one path that leads to the return statement if foo is true, and another if foo is
     * false and bar is true. In the return statement, there is one path that executes a() and another that executes
     * b(). In total, there are 2 * 2 paths that start at {@code entry} and end at the return statement, and 1 path that
    ...
    /**
     * @deprecated Since 7.14.0. Use {@link #NPATH_COMP}, which is available on more nodes, and uses Long instead of
     *             BigInteger.
     */
    @Deprecated
    ...
    /**
     * The relative number of method pairs of a class that access in common at least one attribute of the measured
     * class. TCC only counts direct attribute accesses, that is, only those attributes that are accessed in the body of
     * the method.
     *
     * <p>
     ...
     *
     * <p>
     * TCC is taken to be a reliable cohesion metric for a class. High values (&gt;70%) indicate a class with one basic
     * function, which is hard to break into subcomponents. On the other hand, low values (&lt;50%) may indicate that
     * the class tries to do too much and defines several unrelated services, which is undesirable.
     */
    ...
    /**
     * Sum of the statistical complexity of the operations in the class. We use CYCLO to quantify the complexity of an
     * operation.
     *
     * <p>
    ...
    /**
     * Number of “functional” public methods divided by the total number of public methods. Our definition of
     * “functional method” excludes constructors, getters, and setters.
     *
     * <p>
     * The value is a double between 0 and 1.
     *
     * <p>
     * This metric tries to quantify whether the measured class’ interface reveals more data than behaviour. Low values
     * (less than 30%) indicate that the class reveals much more data than behaviour, which is a sign of poor
     * encapsulation.
     */
