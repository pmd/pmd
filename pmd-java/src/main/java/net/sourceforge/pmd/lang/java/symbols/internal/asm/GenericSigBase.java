...
abstract class GenericSigBase<T extends JTypeParameterOwnerSymbol & AsmStub> {
    /*
     * Signatures must be parsed lazily, because at the point we see them in the
     * file, the enclosing class might not yet have been encountered (and since its
     * type parameters are in scope in the signature we must wait for it).
     */
        ...
        this.lock = new ParseLock(parseLockName) {
            ...
            protected boolean doParse() {
                try {
                    ...
                    throw AssertionUtil.contexted(e).addContextValue("signature", GenericSigBase.this)
            ...
            protected void finishParse(boolean failed) {
                // Mark the bound as ready to parse, after any type annotations have been
                // collected
    ...
    static class LazyClassSignature extends GenericSigBase<ClassStub> {
        ...
        LazyClassSignature(ClassStub ctx, @Nullable String signature, // null if doesn't use generics in header
        ...
        private static @NonNull String sigWithNTypeParams(int observedArity) {
            ...
            switch (observedArity) {
                ...
                default :
                return Stream.iterate(0, i -> i + 1).limit(observedArity).map(i -> "T" + i + OBJECT_BOUND)
    ...
    static class LazyMethodType extends GenericSigBase<ExecutableStub> implements TypeAnnotationReceiver {
        ...
        // - the signature might not contain the thrown exception types (if they do not
        // depend on type variables)
        ...
        // "<TE;>()V^TE;" exceptions: ???
        // <E> void foo(E e) throws Exception; // descriptor "(Ljava.lang.Object;)V"
        // signature "<TE;>(TE;)V" exceptions: [ "java/lang/Exception" ]
        // void foo() throws Exception; // descriptor "()V" signature null exceptions: [
        // "java/lang/Exception" ]
        LazyMethodType(ExecutableStub ctx, @NonNull String descriptor, @Nullable String genericSig,
                @Nullable String[] exceptions, boolean skipFirstParam) {
        ...
        protected void doParse() {
            ...
            if (rawExceptions != null && this.exceptionTypes.isEmpty()) {
                ...
                this.exceptionTypes = Arrays.stream(rawExceptions)
                        .map(ctx.getResolver()::resolveFromInternalNameCannotFail).map(ctx.getTypeSystem()::rawType)
        ...
        public void acceptTypeAnnotation(int typeRefInt, @Nullable TypePath path, SymAnnot annot) {
            ...
            // They shouldn't be applied right now because the descriptor maybe has not been
            // parsed yet.
            if (typeAnnots == null) {
        ...
        /**
         * See {@link MethodVisitor#visitTypeAnnotation(int, TypePath, String, boolean)}
         * for possible values of typeRef sort (they're each case of the switch).
        ...
        boolean acceptAnnotationAfterParse(TypeReference tyRef, @Nullable TypePath path, SymAnnot annot) {
            switch (tyRef.getSort()) {
                ...
                case TypeReference.METHOD_FORMAL_PARAMETER : {
                    ...
                    JTypeMirror annotatedFormal = TypeAnnotationHelper.applySinglePath(parameterTypes.get(idx), path,
                            annot);
                ...
                case TypeReference.THROWS : {
                    ...
                    JTypeMirror annotatedFormal = TypeAnnotationHelper.applySinglePath(exceptionTypes.get(idx), path,
                            annot);
