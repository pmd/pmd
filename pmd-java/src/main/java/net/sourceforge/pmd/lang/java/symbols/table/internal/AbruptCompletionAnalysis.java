...
/**
 * Implementation of {@link #canCompleteNormally(ASTStatement)}, which is used to implement scoping of pattern
 * variables.
 *
 * @see <a href="https://docs.oracle.com/javase/specs/jls/se17/html/jls-14.html#jls-14.22">Java Language
 *      Specification</a>
 */
final class AbruptCompletionAnalysis {
    ...
    /**
     * Returns whether the statement can complete normally, assuming it is reachable. For instance, an expression
     * statement may complete normally, while a return or throw always complete abruptly.
     */
    ...
    /**
     * This visitor implements a reachability analysis: it only visits reachable statements in a given subtree (assuming
     * the first statement visited is reachable). Statements are not visited in a particular order, the only guarantee
     * is that reachable statements are visited. Note that the right-hand side of switch arrow branches is not visited
     * unless it is a statement.
     *
     * The return value of the visitor is whether the visited statement can complete normally.
     */
    static final class ReachabilityVisitor extends JavaVisitorBase<SubtreeState, Boolean> {
        ...
        public Boolean visit(ASTTryStatement node, SubtreeState data) {
            ...
            /*
             * Todo catch clauses are not automatically reachable. A catch block C is reachable iff both of the
             * following are true: - Either the type of C's parameter is an unchecked exception type or Exception or a
             * superclass of Exception, or some expression or throw statement in the try block is reachable and can
             * throw a checked exception whose type is assignment compatible (§5.2) with the type of C's parameter. (An
             * expression is reachable iff the innermost statement containing it is reachable.) - There is no earlier
             * catch block A in the try statement such that the type of C's parameter is the same as, or a subclass of,
    ...
    /**
     * Tracks exploration state of a subtree. A new one is created for independent subtrees (eg in the handling for
     * blocks). Children state instances forward events to their parent (as these events are part of the subtree of
     * their parents).
     */
