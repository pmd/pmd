...
/**
 * Members inserted by the compiler, eg default constructor, etc. They would be
 * absent from the source, but are reflected by the {@link Class}.
 */
public final class ImplicitMemberSymbols {
    ...
    public static JConstructorSymbol defaultCtor(JClassSymbol sym) {
        ...
        // Enum constructors have 2 additional implicit parameters, for the name and
        // ordinal
        // Inner classes have 1 additional implicit param, for the outer instance
        // They are not reflected by the symbol

        int modifiers = sym.isEnum() ? Modifier.PRIVATE : sym.getModifiers() & VISIBILITY_MASK;
    ...
    /**
     * Symbol for a record component accessor. Only synthesized if it is not
     * explicitly declared.
     */
    public static JMethodSymbol recordAccessor(JClassSymbol recordSym, JRecordComponentSymbol recordComponent) {
        // See
        // https://cr.openjdk.java.net/~gbierman/jep359/jep359-20200115/specs/records-jls.html#jls-8.10.3
    ...
    public static JFieldSymbol arrayLengthField(JClassSymbol arraySym) {
        ...
        return new FakeFieldSym(arraySym, "length", Modifier.PUBLIC | Modifier.FINAL, (ts, s) -> ts.INT);
    }
    ...
    private static final class FakeCtorSym extends FakeExecutableSymBase<JConstructorSymbol>
            implements
                JConstructorSymbol {
