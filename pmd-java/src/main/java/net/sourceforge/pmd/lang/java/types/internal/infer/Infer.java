...
public final class Infer {
     ...
     *            Type system
     * @param jdkVersion
     *            JDK version to use. Type inference was changed in Java 8 to propagate the context type.
     * @param logger
     *            Strategy to log failures
     */
    ...
    public PolySite<FunctionalExprMirror> newFunctionalSite(FunctionalExprMirror mirror,
    ...
    // package
    MethodCallSite newCallSite(InvocationMirror expr, @Nullable JTypeMirror expectedType,
            @Nullable MethodCallSite outerSite, @Nullable InferenceContext outerCtx, boolean isSpecificityCheck) {
    ...
    /**
     * Infer lambdas and method references that have a target type: cast contexts, and some assignment contexts (not
     * inferred, not return from lambda).
     */
     ...
     *
     * <p>
     * The returned method type may be {@link TypeSystem#UNRESOLVED_METHOD}, in which case no method is applicable
     * (compile-time error).
     *
     * <p>
     * The returned method type may contain un-instantiated inference variables, which depend on the target type. In
     * that case those variables and their bounds will have been duplicated into the inference context of the [site].
     *
     * <p>
     * The given call site should mention information like the expected return type, to help inference. This should be
     * non-null if we're in an invocation or assignment context, otherwise can be left blank.
     */
    ...
    /**
     * Given a symbol S and a type T which is assumed to be a supertype of some parameterization of S, infer this
     * parameterization.
     */
    ...
    /**
     * Similar to {@link #inferInvocationRecursively(MethodCallSite)} for subexpressions. This never returns a fallback
     * method.
     *
     * <p>
     * A return of {@link #NO_CTDECL} indicates no overload is applicable.
     * <p>
     * A return of {@link #FAILED_INVOCATION} means there is a maximally specific compile-time declaration, but it
     * failed invocation, meaning, it couldn't be linked to its context. If so, the outer inference process must be
     ...
     *
     * <p>
     * The returned method type may be null, in which case no method is applicable (compile-time error).
     */
    private @NonNull MethodCtDecl computeCompileTimeDecl(MethodCallSite site) {

        /*
         * The process starts with a set of candidates and refines it iteratively. Applicability/best applicability are
         * the only ones which needs inference.
         *
    ...
    MethodCtDecl finishInstantiation(MethodCallSite site, MethodCtDecl ctdecl) {
        ...
        return logInference(site, ctdecl.getResolvePhase().asInvoc(), cast(ctdecl.getMethodType()).adaptedMethod());
    }
    ...
    private @NonNull MethodCtDecl logInference(MethodCallSite site, MethodResolutionPhase phase, JMethodSig m) {
        ...
        @Nullable
        JMethodSig candidate = instantiateMethodOrCtor(site, phase, m);
        ...
        if (candidate == null) {
            ...
            return new MethodCtDecl(candidate, phase, site.canSkipInvocation(),
                    OptionalBool.definitely(site.needsUncheckedConversion()), false, site.getExpr());
        }
    }

    private @Nullable JMethodSig instantiateMethodOrCtor(MethodCallSite site, MethodResolutionPhase phase,
            JMethodSig m) {
    ...
    /**
     * Infer type arguments for the given method at the method call. Returns null if no instantiations exist, ie the
     * method is not applicable.
     *
     * @param m
     *            Candidate method
     * @param site
     *            Descriptor of the context of the call.
     * @param phase
     *            Phase in which the method is reviewed
     */
    ...
    private @Nullable JMethodSig instantiateConstructor(JMethodSig cons, MethodCallSite site,
    ...
    private boolean needsAdaptation(CtorInvocationMirror expr, JClassType newType) {
        return expr.isDiamond() || newType.isParameterizedType() // ???
                || expr.isAnonymous();
    }

    /**
     * Transform the constructor of a generic class so that its type parameters mention the type params of the declaring
     * class. This enables diamond inference, we just treat the class type params to infer as additional inference
     * variables.
     *
     * <p>
     * E.g. for
    ...
    private static JMethodSig adaptGenericConstructor(JMethodSig cons, JClassType newType, CtorInvocationMirror expr) {
        ...
        if (newTypeFormals.isEmpty()) {
            ...
            if (consParams.size() > cons.getSymbol().getTypeParameterCount()) {
                ...
                assert consParams
                        .equals(CollectionUtil.concatView(cons.getSymbol().getTypeParameters(), newTypeFormals));
                return adaptedSig;
            }
    ...
    private JMethodSig instantiateMaybeNoInfer(JMethodSig m, MethodCallSite site, MethodResolutionPhase phase) {
        ...
        if (!explicitTargs.isEmpty()) {
            ...
            if (tparams.size() != explicitTargs.size()) {
                ...
                throw ResolutionFailedException.incompatibleTypeParamCount(LOG, site.getExpr(), m, explicitTargs.size(),
                        tparams.size());
            }
            ...
            for (int i = 0; i < tparams.size(); i++) {
                ...
                if (explicit.isConvertibleTo(upperBound).never()) {
                    throw ResolutionFailedException.incompatibleBound(LOG, explicit, upperBound,
                            expr.getExplicitTargLoc(i));
                }
            }
    ...
    /**
     * Perform actual inference. If the method is return-type-polymorphic, then we delegate the solving to the call
     * site's inference context, which knows more, however we add inference vars and their constraints to it. During
     * non-invocation phases, this inference checks for validity but doesn't commit any inferred types.
     */
     ...
     * https://docs.oracle.com/javase/specs/jls/se9/html/jls-18.html#jls-18.5.2 {@code infCtx} must already at the B2
     * state for this method to be called.
     */
    private JMethodSig tryToSolve(JMethodSig m, MethodCallSite site, InferenceContext infCtx,
            MethodResolutionPhase phase) {
    ...
    private boolean shouldPropagateOutwards(JTypeMirror resultType, MethodCallSite target,
            InferenceContext inferenceContext) {
        return !isPreJava8 && !target.getOuterCtx().isEmpty() // enclosing context is a generic method
                ...
                && !(resultType instanceof InferenceVar // no eager instantiation is required (as per 18.5.2)
                        && needsEagerInstantiation((InferenceVar) resultType, target.getExpectedType(),
                                inferenceContext));
    }

    /**
     * Add more constraints on the inference vars based on the expected return type at the call site. This is described
     * in
     *
     ...
     *
     * <p>
     * This binds the ivars of this context to those of the outer context.
     */
    private JTypeMirror addReturnConstraints(InferenceContext infCtx, JMethodSig m, MethodCallSite site) {

        /*
         * Remember: calling stuff like isConvertible or isSubtype adds constraints on the type variables that are found
         * there.
         */
    ...
    /**
     * Returns true if the inference var needs to be instantiated eagerly, as described in JLS§18.5.2.1. (Poly Method
            ...
            for (JTypeMirror aLowerBound : alpha.getBounds(BoundKind.LOWER)) {
                for (JTypeMirror anotherLowerBound : alpha.getBounds(BoundKind.LOWER)) {
                    if (aLowerBound != anotherLowerBound // NOPMD CompareObjectsWithEquals
                            && infCtx.isGround(aLowerBound) && infCtx.isGround(anotherLowerBound)
    ...
    /**
     * Generate bounds on the ivars based on the expected/actual types of the arguments to the call. This is described
     * in
     *
     ...
     *
     * <p>
     * For invocation applicability inference (phases {@link MethodResolutionPhase#STRICT STRICT} through
     * {@link MethodResolutionPhase#VARARGS VARARGS}), only arguments that are
     * {@linkplain ExprOps#isPertinentToApplicability(ExprMirror, JMethodSig, JTypeMirror, InvocationMirror) pertinent
     * to applicability} are considered. Arguments like lambdas do not influence the applicability check beyond checking
     * their basic 'shape' (number of params) to check that the method is
     * {@linkplain #isPotentiallyApplicable(JMethodSig, InvocationMirror) potentially applicable}, which is done very
     * much earlier. So they don't add constraints during those first phases.
     *
     * <p>
     * When we have found an applicable method and are instantiating it (phases
     * {@link MethodResolutionPhase#INVOC_STRICT INVOC_STRICT} through {@link MethodResolutionPhase#INVOC_VARARGS
     * INVOC_VARARGS}), all arguments are considered so as to yield sharper bounds.
     *
     * @param infCtx
     *            Inference context
     * @param m
     ...
     * @param phase
     *            Phase (determines what constraints are allowed)
     */
    private void addArgsConstraints(InferenceContext infCtx, JMethodSig m, MethodCallSite site,
            MethodResolutionPhase phase) {
        LOG.startArgsChecks();
    ...
    /**
     * This corresponds to the attribution of expression compatibility constraints in
     * https://docs.oracle.com/javase/specs/jls/se9/html/jls-18.html#jls-18.2.1 although it's not implemented as
     * described.
     *
     * See {@link ExprCheckHelper#isCompatible(JTypeMirror, ExprMirror)}.
     */
    private void addBoundOrDefer(@Nullable MethodCallSite site, InferenceContext infCtx, MethodResolutionPhase phase,
            @NonNull ExprMirror arg, @NonNull JTypeMirror formalType) {
    ...
    /**
     * Add a compatibility constraint between an exprType and a formalType. This asserts {@code exprType <: formalType},
     * the arg parameter is only used for reporting.
     *
     * <p>
     * This method is called back to by {@link ExprCheckHelper#isCompatible(JTypeMirror, ExprMirror)}.
     */
    void checkConvertibleOrDefer(InferenceContext infCtx, JTypeMirror exprType, JTypeMirror formalType, ExprMirror arg,
            MethodResolutionPhase phase, @Nullable MethodCallSite site) {
    ...
    /**
     * Returns true if the method is potentially applicable to the invocation expression expr, as specified in
     * JLS§15.12.2.1.
     *
     * https://docs.oracle.com/javase/specs/jls/se9/html/jls-15.html#jls-15.12.2.1
     *
     * <p>
     * This assumes the name of the method matches the expression, and the method is accessible.
     *
     * @param m
     *            Method to test
     * @param expr
    ...
    private boolean isPotentiallyApplicable(JMethodSig m, InvocationMirror expr) {
        ...
        if (m.isGeneric() && !expr.getExplicitTypeArguments().isEmpty()
