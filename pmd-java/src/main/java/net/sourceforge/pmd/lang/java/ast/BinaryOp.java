...
/**
 * Represents the operator of an {@linkplain ASTInfixExpression infix
 * expression}. Constants are roughly ordered by precedence, except some of them
...
public enum BinaryOp implements InternalInterfaces.OperatorLike {
    ...
    /**
     * OR {@code "|"} operator. Either logical or bitwise depending on the type of
     * the operands.
     */
    OR("|"),
    /**
     * XOR {@code "^"} operator. Either logical or bitwise depending on the type of
     * the operands.
     */
    XOR("^"),
    /**
     * AND {@code "&"} operator. Either logical or bitwise depending on the type of
     * the operands.
     */
    AND("&"),
    ...
    /**
     * Set of {@code &&} and {@code ||}. Use with
     * {@link JavaAstUtils#isInfixExprWithOperator(JavaNode, Set)}.
     */
    public static final Set<BinaryOp> CONDITIONAL_OPS = CollectionUtil.immutableEnumSet(CONDITIONAL_AND,
            CONDITIONAL_OR);
    /**
     ...
     * {@link JavaAstUtils#isInfixExprWithOperator(JavaNode, Set)}.
     */
    public static final Set<BinaryOp> SHIFT_OPS = CollectionUtil.immutableEnumSet(LEFT_SHIFT, RIGHT_SHIFT,
            UNSIGNED_RIGHT_SHIFT);
     ...
     * {@link Comparator}. Returns a positive integer if this operator has a higher
     * precedence as the argument, zero if they have the same precedence, etc.
     *
    ...
    /**
     * Returns true if this operator has the same relative precedence as the
     * argument. For example, {@link #ADD} and {@link #SUB} have the same
     * precedence.
     *
     * @throws NullPointerException
     *             If the argument is null
    ...
    /**
     * Returns the ops with strictly greater precedence than the given op. This may
     * return an empty set.
     */
    ...
    /**
     * Complement, for boolean operators. Eg for {@code ==}, return {@code !=}, for
     * {@code <=}, returns {@code >}. Returns null if this is another kind of
     * operator.
     */
