...
public final class JavaResolvers {
    ...
    /**
     * Returns true if the given element can be imported in the current file (it's visible & accessible). This is not a
     * general purpose accessibility check and is only appropriate for imports.
     *
     *
     * <p>
     * We consider protected members inaccessible outside of the package they were declared in, which is an
     * approximation but won't cause problems in practice. In an ACU in another package, the name is accessible only
     * inside classes that inherit from the declaring class. But inheriting from a class makes its static members
     * accessible via simple name too. So this will actually be picked up by some other symbol table when in the
     * subclass. Usages outside of the subclass would have made the compilation fail.
     */
    ...
    /**
     * Merges two method scopes, the otherResult is the one of an enclosing class, the inner result is the one inherited
     * from supertypes (which take precedence in case of override equivalence).
     *
     * <p>
     * Non-static methods of the outer result are excluded if the inner scope is static.
     */
    ...
    /**
     * Copy the elements of the input list into the result list, excluding all elements marked by the bitset.
     */
    ...
    /**
     * Resolvers for inherited member types and fields. We can't process methods that way, because there may be
     * duplicates and the equals of {@link JMethodSymbol} is not reliable for now (cannot differentiate overloads). But
     * also, usually a subset of methods is used in a subclass, and it's ok performance-wise to process them on-demand.
     */
    static Pair<NameResolver<JTypeMirror>, NameResolver<JVariableSig>> inheritedMembersResolvers(JClassType t) {
        Pair<ShadowChainBuilder<JTypeMirror, ?>.ResolverBuilder, ShadowChainBuilder<JVariableSig, ?>.ResolverBuilder> builders =
                hidingWalkResolvers(t, t, t.getSymbol().getPackageName(), true, /* onlyStatic: */false,
    ...
    /**
     * A general-purpose accessibility check, which can be used if you know in advance whether the context is a
     * supertype of the class (ie, whether the sym is accessible if "protected").
     *
     * @param nestRoot
    ...
    /**
     * Whether the given sym is accessible in some type T, given the 'nestRoot' of T, and whether T is a subtype of the
     * class declaring 'sym'. This is a general purpose accessibility check, albeit a bit low-level (but only needs
     * subtyping to be computed once).
     */
    ...
    /**
     * Produce a name resolver that resolves member classes with the given name declared or inherited by the given type.
     * Each access may perform a hierarchy traversal, but this handles hidden and ambiguous declarations nicely.
     *
     * @param c
     *            Class to search
     * @param access
