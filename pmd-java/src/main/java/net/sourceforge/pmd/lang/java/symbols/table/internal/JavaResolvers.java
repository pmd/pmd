...
public final class JavaResolvers {
    ...
    /**
     * Returns true if the given element can be imported in the current file (it's
     * visible & accessible). This is not a general purpose accessibility check and
     * is only appropriate for imports.
     *
     *
     * <p>
     * We consider protected members inaccessible outside of the package they were
     * declared in, which is an approximation but won't cause problems in practice.
     * In an ACU in another package, the name is accessible only inside classes that
     * inherit from the declaring class. But inheriting from a class makes its
     * static members accessible via simple name too. So this will actually be
     * picked up by some other symbol table when in the subclass. Usages outside of
     * the subclass would have made the compilation fail.
     */
    ...
    @NonNull
    static NameResolver<JTypeMirror> moduleImport(Set<String> moduleNames, final SymbolResolver symbolResolver,
    ...
    static NameResolver<JTypeMirror> importedOnDemand(Set<String> lazyImportedPackagesAndTypes,
            final SymbolResolver symResolver, final String thisPackage) {
        return new SingleNameResolver<JTypeMirror>() {
            @Nullable
            @Override
            public JTypeMirror resolveFirst(String simpleName) {
                for (String pack : lazyImportedPackagesAndTypes) {
                    // here 'pack' may be a package or a type name, so we must resolve by canonical
                    // name
    ...
    static NameResolver<JMethodSig> subtypeMethodResolver(JClassType t) {
        ...
        return new NameResolver<JMethodSig>() {
            ...
            public @NonNull List<JMethodSig> resolveHere(String simpleName) {
                return t.streamMethods(it -> it.nameEquals(simpleName) && isAccessibleIn(nestRoot, it, true) // fetch
                                                                                                             // protected
                                                                                                             // methods
                        && isNotStaticInterfaceMethod(it)).collect(OverloadSet.collectMostSpecific(t)); // remove
                                                                                                        // overridden,
                                                                                                        // hidden
                                                                                                        // methods
            }

            // Static interface methods are not inherited and are in fact not in scope in
            // the subtypes.
    ...
    static NameResolver<JMethodSig> staticImportMethodResolver(JClassType container, @NonNull String accessPackageName,
        ...
        return new NameResolver<JMethodSig>() {
            ...
            public @NonNull List<JMethodSig> resolveHere(String simpleName) {
                ...
                return container.streamMethods(it -> Modifier.isStatic(it.getModifiers()) && it.nameEquals(simpleName)
                ...
                // the relevant contexts.
                        && canBeImportedIn(accessPackageName, it)).collect(OverloadSet.collectMostSpecific(container)); // remove
                                                                                                                        // overridden,
                                                                                                                        // hidden
                                                                                                                        // methods
            }
    ...
    static NameResolver<FieldSig> staticImportFieldResolver(JClassType containerType, @NonNull String accessPackageName,
            String importedSimpleName) {
    ...
    static NameResolver<JClassType> staticImportClassResolver(JClassType containerType,
            @NonNull String accessPackageName, String importedSimpleName) {
    ...
    static NameResolver<JMethodSig> staticImportOnDemandMethodResolver(JClassType container,
        ...
        return new NameResolver<JMethodSig>() {
            ...
            public @NonNull List<JMethodSig> resolveHere(String simpleName) {
                return container
                        .streamMethods(it -> Modifier.isStatic(it.getModifiers()) && it.nameEquals(simpleName)
     ...
     *
     * <p>
     * Non-static methods of the outer result are excluded if the inner scope is
     * static.
     */
    private static List<JMethodSig> methodMerger(boolean inStaticType, List<JMethodSig> myResult,
            List<JMethodSig> otherResult) {
    ...
    /**
     * Copy the elements of the input list into the result list, excluding all
     * elements marked by the bitset.
     */
    ...
    /**
     * Resolvers for inherited member types and fields. We can't process methods
     * that way, because there may be duplicates and the equals of
     * {@link JMethodSymbol} is not reliable for now (cannot differentiate
     * overloads). But also, usually a subset of methods is used in a subclass, and
     * it's ok performance-wise to process them on-demand.
     */
    static Pair<NameResolver<JTypeMirror>, NameResolver<JVariableSig>> inheritedMembersResolvers(JClassType t) {
        Pair<ShadowChainBuilder<JTypeMirror, ?>.ResolverBuilder, ShadowChainBuilder<JVariableSig, ?>.ResolverBuilder> builders = hidingWalkResolvers(
    ...
    static Pair<ShadowChainBuilder<JTypeMirror, ?>.ResolverBuilder, ShadowChainBuilder<JVariableSig, ?>.ResolverBuilder> importOnDemandMembersResolvers(
            JClassType t, @NonNull String accessPackageName) {
        return hidingWalkResolvers(t, null, accessPackageName, false, /* onlyStatic: */ true,
                JUST_SELF /* include self members */);
    }

    private static Pair<ShadowChainBuilder<JTypeMirror, ?>.ResolverBuilder, ShadowChainBuilder<JVariableSig, ?>.ResolverBuilder> hidingWalkResolvers(
            JClassType t, @Nullable JClassType accessType, @NonNull String accessPackageName,
            boolean accessIsSubtypeOfOwner, boolean onlyStatic, SuperTypesEnumerator enumerator) {
        ...
        for (JClassType next : enumerator.iterable(t)) {
            walkSelf(next, isFieldAccessible, isTypeAccessible, fields, types, HashTreePSet.empty(),
                    HashTreePSet.empty());
        }
    ...
    private static void walkSelf(JClassType t, Predicate<? super JVariableSig> isFieldAccessible,
            Predicate<? super JClassType> isTypeAccessible, ShadowChainBuilder<JVariableSig, ?>.ResolverBuilder fields,
            ShadowChainBuilder<JTypeMirror, ?>.ResolverBuilder types,
            // persistent because may change in every path of the recursion
            final PSet<String> hiddenFields, final PSet<String> hiddenTypes) {
        ...
        PSet<String> hiddenTypesInSup = processDeclarations(types, hiddenTypes, isTypeAccessible,
                t.getDeclaredClasses());
        PSet<String> hiddenFieldsInSup = processDeclarations(fields, hiddenFields, isFieldAccessible,
                t.getDeclaredFields());
    ...
    private static <S> PSet<String> processDeclarations(ShadowChainBuilder<? super S, ?>.ResolverBuilder builder,
            PSet<String> hidden, Predicate<? super S> isAccessible, List<? extends S> syms) {
        for (S inner : syms) {
    ...
    /**
     * A general-purpose accessibility check, which can be used if you know in
     * advance whether the context is a supertype of the class (ie, whether the sym
     * is accessible if "protected").
     *
     * @param nestRoot
     *            Root enclosing type for the context of the reference
     * @param sym
     *            Symbol to test
     */
    public static boolean isAccessibleIn(@NonNull JClassSymbol nestRoot, JAccessibleElementSymbol sym,
            boolean isOwnerASupertypeOfContext) {
        return isAccessibleIn(nestRoot, nestRoot.getPackageName(), sym, isOwnerASupertypeOfContext);
    }

    /**
     * Whether the given sym is accessible in some type T, given the 'nestRoot' of
     * T, and whether T is a subtype of the class declaring 'sym'. This is a general
     * purpose accessibility check, albeit a bit low-level (but only needs subtyping
     * to be computed once).
     */
    private static boolean isAccessibleIn(@Nullable JClassSymbol nestRoot, @NonNull String packageName,
            JAccessibleElementSymbol sym, boolean isOwnerASupertypeOfContext) {
        ...
        switch (modifiers) {
            ...
            default :
            // TODO this is reachable for invalid declarations, like a private field of an
            // interface
            throw AssertionUtil.shouldNotReachHere(
                    "private field of an interface? " + sym + ", modifiers: " + Modifier.toString(sym.getModifiers()));
        }
    }

    /**
     * Produce a name resolver that resolves member classes with the given name
     * declared or inherited by the given type. Each access may perform a hierarchy
     * traversal, but this handles hidden and ambiguous declarations nicely.
     *
     * @param c
     *            Class to search
     * @param access
     *            Context of where the declaration is referenced
     * @param name
     *            Name of the class to find
     */
    public static NameResolver<JClassType> getMemberClassResolver(JClassType c, @NonNull String accessPackageName,
            @Nullable JClassSymbol access, String name) {
        return getNamedMemberResolver(c, access, accessPackageName, JClassType::getDeclaredClass, name,
                JClassType::getSymbol, SymTableFactory.TYPES);
    }

    public static NameResolver<FieldSig> getMemberFieldResolver(JClassType c, @NonNull String accessPackageName,
            @Nullable JClassSymbol access, String name) {
        return getNamedMemberResolver(c, access, accessPackageName, JClassType::getDeclaredField, name,
                FieldSig::getSymbol, SymTableFactory.VARS);
    }

    private static <S> NameResolver<S> getNamedMemberResolver(JClassType c, @Nullable JClassSymbol access,
            @NonNull String accessPackageName, BiFunction<? super JClassType, String, ? extends S> getter, String name,
    ...
    private static <S> void walkForSingleName(JClassType t, Predicate<? super S> isAccessible, String name,
            BiFunction<? super JClassType, String, ? extends S> getter,
            ShadowChainBuilder<? super S, ?>.ResolverBuilder builder, final PSet<String> hidden) {

        PSet<String> hiddenInSup = processDeclarations(builder, hidden, isAccessible,
                listOfNotNull(getter.apply(t, name)));
