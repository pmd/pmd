...
public class CognitiveComplexityRule extends AbstractJavaRulechainRule {
    ...
    private static final PropertyDescriptor<Integer> REPORT_LEVEL_DESCRIPTOR = PropertyFactory
            .intProperty("reportLevel").desc("Cognitive Complexity reporting threshold").require(positive())
    ...
    private Object visitMethod(ASTExecutableDeclaration node, Object data) {
        ...
        if (cognitive >= reportLevel) {
            asCtx(data).addViolation(node, node instanceof ASTMethodDeclaration ? "method" : "constructor",
                    PrettyPrintingUtil.displaySignature(node), String.valueOf(cognitive), String.valueOf(reportLevel));
        }
