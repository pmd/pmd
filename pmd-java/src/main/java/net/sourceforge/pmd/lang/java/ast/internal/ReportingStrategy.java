 ...
 * {@link LanguageLevelChecker}. For example,
 * {@link ReportingStrategy#reporterThatThrows()} produces a checker that throws
 * a parse exception. It would be trivial to make eg a checker that eg collects
 * all warnings instead of failing on the first one.
 *
 * @param <T>
...
public interface ReportingStrategy<T> {
    ...
    /**
     * Report that a node violates a language feature. This doesn't have to throw an
     * exception, we could also just warn, or accumulate into the parameter.
     */
    void report(Node node, String message, T acc);

    /**
     * Creates a reporter that throws a {@link ParseException} when the first error
     * is reported.
     */
