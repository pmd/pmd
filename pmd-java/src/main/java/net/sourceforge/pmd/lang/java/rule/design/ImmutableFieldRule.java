...
public class ImmutableFieldRule extends AbstractJavaRulechainRule {
    ...
    private static final Set<String> INVALIDATING_CLASS_ANNOT = setOf("lombok.Builder", "lombok.Data", "lombok.Setter",
            "lombok.Value");
    ...
    private boolean defaultValueDoesNotReachEndOfCtor(DataflowResult dataflow, ASTVariableId varId) {
        ...
        // no killer isFieldAssignmentAtEndOfCtor => the field is assigned on all code
        // paths
    ...
    private boolean isReassignedOnSomeCodePath(DataflowResult dataflow, AssignmentEntry anAssignment) {
        // Ie, return whether there exists an assignment that overwrites the given
        // assignment,
        // and simultaneously is not unbound (=happens for sure). Unbound assignments
        // are introduced by the
        // dataflow pass to help analysis, but are overly conservative. They represent
        // the
        // "final value" of a field when a ctor ends, and also the value a field may
        // have
        // been set to when an instance method is called (we don't know whether the
        // method
