...
public class PreserveStackTraceRule extends AbstractJavaRulechainRule {
    ...
    private boolean exprConsumesException(Set<ASTVariableId> exceptionParams, ASTExpression expr, boolean mayBeSelf) {
        if (expr instanceof ASTConstructorCall) {
            ...
            // Peel out a type pattern variable in case this conditional is an instanceof
            // pattern
            ...
            for (ASTNamedReferenceExpr usage : decl.getLocalUsages()) {
                ...
                if (JavaAstUtils.followingCallChain(usage)
                        .any(it -> consumesExceptionNonRecursive(exceptionParams, it))) {
                    return true;
                }
            }
    ...
    private boolean assignmentRhsConsumesException(Set<ASTVariableId> exceptionParams, ASTVariableId lhsVariable,
        ...
        if (usage.getIndexInParent() == 0) {
            ...
            boolean rhsIsSelfReferential = NodeStream.of(assignmentRhs).descendantsOrSelf()
