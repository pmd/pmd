...
/**
 * Matches a method or constructor call against a particular overload. Use {@link #parse(String)} to create one. For
 * example,
 *
 * <pre>
 ...
 *
 * <p>
 * The receiver matcher (first half) is matched against the static type of the <i>receiver</i> of the call, and not the
 * declaration site of the method, unless the called method is static, or a constructor.
 *
 * <p>
 * The parameters are matched against the declared parameters types of the called overload, and not the actual argument
 * types. In particular, for vararg methods, the signature should mention a single parameter, with an array type.
 *
 * <p>
 * For example {@code Integer.valueOf('0')} will be matched by {@code _#valueOf(int)} but not {@code _#valueOf(char)},
 * which is an overload that does not exist (the char is widened to an int, so the int overload is selected).
 *
...
public final class InvocationMatcher {
    ...
    /**
     * Returns true if the call matches this matcher. This means, the called overload is the one identified by the
     * argument matchers, and the actual qualifier type is a subtype of the one mentioned by the qualifier matcher.
     */
    ...
    /**
     * Parses a {@link CompoundInvocationMatcher} which matches any of the provided matchers.
     *
     * @param first
    ...
    private static final class TypeMatcher {
        ...
        boolean matches(JTypeMirror type, boolean exact) {
            return name == null || (exact ? TypeTestUtil.isExactlyAOrAnon(name, type) == OptionalBool.YES
                    : TypeTestUtil.isA(name, type));
        }
    }

    /**
     * A compound of several matchers (logical OR). Get one from {@link InvocationMatcher#parseAll(String, String...)};
     */
