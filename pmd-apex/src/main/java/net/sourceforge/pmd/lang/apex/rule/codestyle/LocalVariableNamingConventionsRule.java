...
public class LocalVariableNamingConventionsRule extends AbstractNamingConventionsRule {
    ...
    private static final PropertyDescriptor<Pattern> FINAL_REGEX = prop(
                    "finalLocalPattern", "final local variable", DESCRIPTOR_TO_DISPLAY_NAME)
            .defaultValue(CAMEL_CASE)
            .build();

    private static final PropertyDescriptor<Pattern> LOCAL_REGEX = prop(
                    "localPattern", "local variable", DESCRIPTOR_TO_DISPLAY_NAME)
            .defaultValue(CAMEL_CASE)
            .build();
    ...
    public Object visit(ASTVariableDeclaration node, Object data) {
        if (node.ancestors(ASTVariableDeclarationStatements.class)
                .first()
                .getModifiers()
                .isFinal()) {
