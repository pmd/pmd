...
public class UnusedLocalVariableRule extends AbstractApexRule {
    ...
    public Object visit(ASTVariableDeclaration node, Object data) {
        ...
        ASTBlockStatement variableContext =
                node.ancestors(ASTBlockStatement.class).first();
        ...
        potentialUsages.addAll(
                variableContext.descendants(ASTVariableExpression.class).toList());
        // Reference expressions catch things like the `a` in `a.foo()` or in `:a.Id` (Binding Expression)
        potentialUsages.addAll(
                variableContext.descendants(ASTReferenceExpression.class).toList());
    ...
    private Collection<String> findBindingsInSOSLQueries(ASTBlockStatement variableContext) {
        return variableContext
                .descendants(ASTSoslExpression.class)
                .toStream()
    ...
    private List<String> findBindingsInSOQLStringLiterals(ASTBlockStatement variableContext) {
        List<ASTMethodCallExpression> methodCalls = variableContext
                .descendants(ASTMethodCallExpression.class)
                .filter(m ->
                        DATABASE_QUERY_METHODS.contains(m.getFullMethodName().toLowerCase(Locale.ROOT)))
                .collect(Collectors.toList());

        List<String> stringLiterals = new ArrayList<>();

        for (ASTMethodCallExpression databaseMethodCall : methodCalls) {
            stringLiterals.addAll(databaseMethodCall
            ...
            databaseMethodCall.descendants(ASTVariableExpression.class).forEach(variableUsage -> {
                ...
                variableContext
                        .descendants(ASTVariableExpression.class)
