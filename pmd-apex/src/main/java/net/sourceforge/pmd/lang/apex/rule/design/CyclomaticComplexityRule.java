    ...
    public Object visit(ASTUserClass node, Object data) {
        ...
        if (ApexMetrics.WEIGHED_METHOD_COUNT.supports(node)) {
            ...
            if (classWmc >= getProperty(CLASS_LEVEL_DESCRIPTOR)) {
                int classHighest = (int) MetricsUtil.computeStatistics(ApexMetrics.CYCLO, node.getMethods())
                        .getMax();

                String[] messageParams = {
                    "class", node.getSimpleName(), " total", classWmc + " (highest " + classHighest + ")",
                };
    ...
    public final Object visit(ASTMethod node, Object data) {
        ...
        if (ApexMetrics.CYCLO.supports(node)) {
            ...
            if (cyclo >= getProperty(METHOD_LEVEL_DESCRIPTOR)) {
                ...
                asCtx(data).addViolation(node, opType, node.getQualifiedName().getOperation(), "", "" + cyclo);
            }
        }
