...
/**
 * PMDRecord for a Package
 * creates Files when instantiated
 * 
...
public class PackageRecord extends PMDRecord {
		...
		try {
			...
			for (int k=0; k<javaUnits.length; k++) {
				IResource javaResource = 
					javaUnits[k].getCorrespondingResource();
	...
	public PMDRecord addResource(IResource resource) {
		ICompilationUnit unit = 
			packageFragment.getCompilationUnit(resource.getName());
	...
	public String getName() {
		...
		if (packageFragment.isDefaultPackage())
			return PMDPlugin.getDefault().getMessage(
				PMDConstants.MSGKEY_VIEW_DEFAULT_PACKAGE);
