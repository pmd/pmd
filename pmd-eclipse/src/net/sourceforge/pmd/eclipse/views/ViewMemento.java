...
/**
 * Provides Functions to save the State of a View
 * during a Session, even when the view is cloes and re-opened
 * Saves the State in a XML-File in the Plugins-Path 
 * (Standard: .metadata in the workspace)
 * 
...
public class ViewMemento {
	...
	protected final static String XML_PREFIX = 
	...
	/** Constructor
	 * Searches for the XML-File, where the Memento should be saved 
	 * and creates it if there is none
	 * 
	 * @param type, a String identifying the View, 
	 * used for the File's Name
	 */
	public ViewMemento(String type) {
		path = PMDPlugin.getDefault().getStateLocation();
		file = new File(path.toString(), type);
		
		// we check for an existing XML-File
		// and create one, if needed		
		if ((!file.exists())
				|| (checkForXMLFile(file) == false))
			createNewFile(file);
		...
		try {
			...
			PMDPlugin.getDefault().logError(
				...
				path.toString() + "/" + type + " in " + this.toString(),
				fnfe);
	 ...
	 * 
	 * @param file
	 * @return true, if the File is a XML-File we can use, 
	 * false otherwise
	 */
	protected boolean checkForXMLFile(File file) {
		try {
			...
			PMDPlugin.getDefault().logError(
				PMDConstants.MSGKEY_ERROR_FILE_NOT_FOUND +
				file.toString() + " in " + this.toString(), fnfe);
		} catch (IOException ioe) {
			PMDPlugin.getDefault().logError(
				PMDConstants.MSGKEY_ERROR_IO_EXCEPTION + 
				this.toString(), ioe);
	...
	private IMemento getAttribute(String name) {
		IMemento[] mementos = 
			memento.getChildren(ATTRIBUTE_PREFIX);
		IMemento mem = null;
	...
	/**
	 * puts an ArrayList into a Memento,
	 * the List is changed into a seperated String
	 * 
	 * @param key
	 * @param valueList
	 */
	...
	public ArrayList getStringList(String key) {
		...
		ArrayList valuelist = new ArrayList(
			Arrays.asList(valueString.split(LIST_SEPARATOR)));
		return valuelist;
	}
}
