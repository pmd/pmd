...
/**
 * Enables the QuickFix Action for a Marker
...
public class QuickFixAction extends ViolationSelectionAction {
    ...
    public QuickFixAction(TableViewer viewer) {
        ...
        setImageDescriptor(PMDPlugin.getDefault().getImageDescriptor(
        	PMDPlugin.ICON_BUTTON_QUICKFIX));
        setText(PMDPlugin.getDefault().getMessage(
        	PMDConstants.MSGKEY_VIEW_ACTION_QUICKFIX));
        setToolTipText(PMDPlugin.getDefault().getMessage(
        	PMDConstants.MSGKEY_VIEW_TOOLTIP_QUICKFIX));
    }
    ...
    public void run() {
        ...
        IMarkerResolution resolutions[] = 
        	IDE.getMarkerHelpRegistry().getResolutions(selectedMarkers[0]);
        if (resolutions.length != 0) {
            ...
            if (dialog.open() == Dialog.OK) {
                ...
                if ((result != null) && (result.length > 0)) {
                    IMarkerResolution selectedResolution = 
                    	(IMarkerResolution) result[0];
