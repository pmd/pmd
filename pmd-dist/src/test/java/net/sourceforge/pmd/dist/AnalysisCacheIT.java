...
class AnalysisCacheIT extends AbstractBinaryDistributionTest {
    ...
    void testPmdCachedResultMatches() throws Exception {
        ...
        ExecutionResult result = PMDExecutor.runPMD(
        ...
        result.assertExitCode(4)
                .assertReport(
                        containsString(srcDir + File.separator + "JumbledIncrementer.java:8:\tJumbledIncrementer:\t"));

        // rerun from cache
        ExecutionResult resultFromCache = PMDExecutor.runPMD(
                createTemporaryReportFile(),
                tempDir,
                "-d",
                srcDir,
    ...
    void testPmdCachedResultsAreRelativized() throws Exception {
        ...
        ExecutionResult resultFromCache = PMDExecutor.runPMD(
                ...
                tempDir,
                "-d",
                Paths.get(".").toAbsolutePath().relativize(Paths.get(srcDir)).toString(),
                "-R",
                "src/test/resources/rulesets/sample-ruleset.xml",
                "-f",
                "text",
        ...
        result.assertExitCode(4)
                .assertReport(containsString(
                        "src/test/resources/sample-source/java/JumbledIncrementer.java:8:\tJumbledIncrementer:\t"
                                .replace('/', File.separatorChar)));
        resultFromCache.assertNoErrorInReport(
