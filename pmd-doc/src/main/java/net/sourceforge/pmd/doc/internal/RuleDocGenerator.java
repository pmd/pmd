...
public class RuleDocGenerator {
    ...
    private static final String GENERATED_WARNING =
            "<!-- DO NOT EDIT THIS FILE. This file is generated from file ${source}. -->";
    ...
    private static final String RULESET_INDEX_FILENAME_PATTERN =
            "docs/pages/pmd/rules/${language.tersename}/${ruleset.name}.md";
    private static final String RULESET_INDEX_PERMALINK_PATTERN =
            "pmd_rules_${language.tersename}_${ruleset.name}.html";

    private static final String DEPRECATION_LABEL_SMALL =
    ...
    private static final String DEPRECATION_LABEL =
    ...
    private Map<Language, List<RuleSet>> sortRulesets(List<RuleSet> rulesets) {
        SortedMap<Language, List<RuleSet>> rulesetsByLanguage = rulesets.stream()
     ...
     * @throws IOException
     */
    private void generateLanguageIndex(
            Map<Language, List<RuleSet>> rulesets, Map<Language, List<RuleSet>> sortedAdditionalRulesets)
            throws IOException {
        for (Map.Entry<Language, List<RuleSet>> entry : rulesets.entrySet()) {
            ...
            lines.add("summary: Index of all built-in rules available for "
            ...
            for (RuleSet ruleset : entry.getValue()) {
                ...
                for (Rule rule : getSortedRules(ruleset)) {
                    ...
                    if (rule instanceof RuleReference) {
                        ...
                        if (ruleset.getFileName()
                                .equals(ref.getRuleSetReference().getRuleSetFileName())) {
                            ...
                            String otherLink = RULESET_INDEX_PERMALINK_PATTERN
                                    ...
                                    .replace(
                                            "${ruleset.name}",
                                            RuleSetUtils.getRuleSetFilename(
                                                    ref.getRuleSetReference().getRuleSetFileName()));
                            lines.add("*   [" + rule.getName() + "](" + link + "#"
                                    + rule.getName().toLowerCase(Locale.ROOT) + "): "
                                    + DEPRECATION_LABEL_SMALL
                ...
                for (RuleSet ruleset : additionalRulesetsForLanguage) {
                    ...
                    for (Rule rule : getSortedRules(ruleset)) {
                        ...
                        if (resolvedRule instanceof RuleReference) {
                            ...
                            rules.append(otherLink)
                                    .append("#")
                                    .append(ref.getRule().getName().toLowerCase(Locale.ROOT))
                                    .append(")");
                        } else {
    ...
    private static String getShortRuleDescription(Rule rule) {
        String htmlEscaped = StringEscapeUtils.escapeHtml4(StringUtils.abbreviate(
                StringUtils.stripToEmpty(rule.getDescription()
                        .replaceAll("\n+|\r+", " ")
                        .replaceAll("\\|", "\\\\|")
                        .replaceAll("`", "'")
                        .replaceAll("\\*", "")),
            ...
            for (RuleSet ruleset : rulesets) {
                ...
                for (Rule rule : ruleset.getRules()) {
                    ...
                    String relativeSourceFilename =
                            ruleClass.replaceAll("\\.", Matcher.quoteReplacement(File.separator)) + ".java";
