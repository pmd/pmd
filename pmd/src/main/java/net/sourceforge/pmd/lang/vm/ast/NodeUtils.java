...
public class NodeUtils {
     ...
     */
    public static StrBuilder getSpecialText(final Token t) {
    ...
    /**
     * Utility method to interpolate context variables into string literals. So that the following will work:
     * 
     * #set $name = "candy" $image.getURI("${name}.jpg")
     * 
     * And the string literal argument will be transformed into "candy.jpg" before the method is executed.
     * 
    ...
    public static String interpolate(final String argStr, final Context vars) throws MethodInvocationException {
        ...
        final StrBuilder argBuf = new StrBuilder();

        for (int cIdx = 0, is = argStr.length(); cIdx < is;) {
            char ch = argStr.charAt(cIdx);

            if (ch == '$') {
                final StrBuilder nameBuf = new StrBuilder();
