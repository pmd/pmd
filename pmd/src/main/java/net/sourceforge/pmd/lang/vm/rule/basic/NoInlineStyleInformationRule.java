...
/**
 * This rule checks that no "style" elements (like <B>, <FONT>, ...) are used, and that no "style" attributes (like
 * "font", "size", "align") are used.
 * 
 * @author pieter_van_raemdonck
 */
public class NoInlineStyleInformationRule extends AbstractVmRule {
     ...
     */
    private static final Set<String> STYLE_ELEMENT_NAMES = CollectionUtil.asSet(new String[] {"B", "I", "FONT",
            "BASEFONT", "U", "CENTER"});

    /**
     * List of HTML element-names that can have attributes defining style.
     */
    private static final Set<String> ELEMENT_NAMES_THAT_CAN_HAVE_STYLE_ATTRIBUTES = CollectionUtil.asSet(new String[] {
            "P", "TABLE", "THEAD", "TBODY", "TFOOT", "TR", "TD", "COL", "COLGROUP"});

    /**
     * List of attributes that define style when they are attributes of HTML elements with names in
     * ELEMENT_NAMES_THAT_CAN_HAVE_STYLE_ATTRIBUTES.
     */
    private static final Set<String> STYLE_ATTRIBUTES = CollectionUtil.asSet(new String[] {"STYLE", "FONT", "SIZE",
            "COLOR", "FACE", "ALIGN", "VALIGN", "BGCOLOR"});
    ...
    /**
     * Checks whether the attributeNode argument is a style attribute of a HTML element that can have style attributes.
     * 
    ...
    private boolean isStyleAttribute(final ASTAttribute attributeNode) {
        if (STYLE_ATTRIBUTES.contains(attributeNode.getName().toUpperCase())) {
            if (attributeNode.jjtGetParent() instanceof ASTElement) {
                final ASTElement parent = (ASTElement) attributeNode.jjtGetParent();
